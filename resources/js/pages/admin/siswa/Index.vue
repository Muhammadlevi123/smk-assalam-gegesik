...
interface Props {
    ...
    kelasList:       Array<{ id: number; nama: string }>;
...
const searchForm = useForm({
    ...
    kelulusan:     props.filters?.kelulusan     || '',
...
const debouncedSearch = debounce(() => {
    searchForm.get('/admin/siswa', { preserveState: true, preserveScroll: true });
}, 300);
...
const confirmDelete = () => {
    ...
    deleteForm.delete(`/admin/siswa/${selectedSiswa.value.id}`, {
        preserveScroll: true,
        onSuccess: () => {
...
// ── Helper utama ──────────────────────────────────────────────────
// Jika ada kelulusan terisi → tampilkan kelulusan
// Jika semua kelulusan NULL → tampilkan status tahun terbaru
const getSiswaStatusDetail = (siswa: Siswa): { label: string; color: string } => {
    if (!siswa.tahunAjaranStatus || siswa.tahunAjaranStatus.length === 0)
        return {
            label: 'Aktif',
            color: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
        };

    const sorted = [...siswa.tahunAjaranStatus]
        .sort((a, b) => parseInt(b.tahun.split('/')[0]) - parseInt(a.tahun.split('/')[0]));

    const denganKelulusan = sorted.find(
        ta => ta.pivot.kelulusan !== null && ta.pivot.kelulusan !== undefined && ta.pivot.kelulusan !== ''
    );

    if (denganKelulusan) {
        const kelulusan = denganKelulusan.pivot.kelulusan as string;
        return {
            label: kelulusan,
            color: kelulusan === 'Lulus'
                ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300'
                : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300',
        };
    }

    const statusTerbaru = sorted[0].pivot.status;
    ...
    return {
        label: statusTerbaru,
        color: colorMap[statusTerbaru] ?? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
    };
};
...
const hasActiveFilters = computed(() =>
    !!(props.filters?.search || props.filters?.kelas || props.filters?.jenis_kelamin ||
       props.filters?.status || props.filters?.kelulusan || props.filters?.tahun_ajaran ||
       props.filters?.angkatan)
);

const countLakiLaki  = computed(() => props.siswa.data.filter(s => s.jenis_kelamin === 'Laki-laki').length);
const countPerempuan = computed(() => props.siswa.data.filter(s => s.jenis_kelamin === 'Perempuan').length);
const countAktif     = computed(() => props.siswa.data.filter(s => getSiswaStatusDetail(s).label === 'Aktif').length);
const countLulus     = computed(() => props.siswa.data.filter(s => getSiswaStatusDetail(s).label === 'Lulus').length);
        ...
        <div class="bg-gray-50/50 dark:bg-gray-950/50 min-h-screen">
            <div class="mx-auto max-w-7xl space-y-6 px-3 py-4 sm:px-4 sm:py-6 lg:px-8 lg:py-8">
                ...
                <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between lg:items-center">
                    <div class="space-y-2">
                        <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-3xl">Data Siswa</h1>
                        <p class="text-sm text-gray-600 dark:text-gray-400 sm:text-base">Kelola data siswa sekolah dan informasi akademik</p>
                        <div class="mt-3 flex flex-wrap items-center gap-3 text-xs text-gray-500 dark:text-gray-400 sm:gap-6 sm:text-sm">
                            <div class="flex items-center gap-2">
                                ...
                                <span>{{ siswa.total }} Total Siswa</span>
                            </div>
                            <div class="flex items-center gap-2">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <span>{{ countLulus }} Lulus</span>
                            </div>
                            <div v-if="hasActiveFilters" class="flex items-center gap-2">
                                <div class="h-2 w-2 rounded-full bg-orange-500"></div>
                                <span>{{ siswa.data.length }} Terfilter</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <Link :href="route('admin.siswa.create')"
                        ...
                        <span class="sm:inline">Tambah Siswa</span>
                    </Link>
                </div>
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-12 xl:gap-6">
                            ...
                            <div class="sm:col-span-2 lg:col-span-3 xl:col-span-4">
                                ...
                                <div class="relative">
                                    ...
                                    <input v-model="searchForm.search" type="text" placeholder="Cari berdasarkan nama, NIS, angkatan..."
                            ...
                            <div class="xl:col-span-2">
                                ...
                                <div class="relative">
                                    <select v-model="searchForm.kelas"
                                        ...
                                        <option v-for="k in kelasList" :key="k.id" :value="k.nama">{{ k.nama }}</option>
                                    </select>
                            ...
                            <!-- Kelulusan -->
                            <div class="xl:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Kelulusan</label>
                                <div class="relative">
                                    <select v-model="searchForm.kelulusan"
                                        class="block w-full appearance-none rounded-lg border-0 bg-gray-50 py-2.5 pl-3 pr-8 text-sm text-gray-900 ring-1 ring-inset ring-gray-200 focus:bg-white focus:ring-2 focus:ring-blue-600 dark:bg-gray-800 dark:text-white dark:ring-gray-700 dark:focus:bg-gray-700 sm:py-3 sm:pl-4 sm:pr-10 sm:rounded-xl">
                                        <option value="">Semua</option>
                ...
                <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900 sm:rounded-2xl overflow-hidden">
                    <div class="border-b border-gray-100 bg-gray-50/50 px-4 py-3 dark:border-gray-800 dark:bg-gray-800/50 sm:px-6 sm:py-4">
                        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-0">
                            <p class="text-sm font-medium text-gray-900 dark:text-white">
                                Menampilkan {{ siswa.data.length }} dari {{ siswa.total }} siswa
                            </p>
                    ...
                    <!-- Desktop Table -->
                    <div v-if="siswa.data.length > 0" class="hidden lg:block overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50/80 dark:bg-gray-800/50">
                                <tr>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-gray-400">Siswa</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-gray-400">NIS</th>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-gray-400">Angkatan</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-gray-400">Alamat</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-900 dark:divide-gray-700">
                                <tr v-for="item in siswa.data" :key="item.id"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white">{{ item.nis }}</div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900 dark:text-white max-w-xs truncate" :title="item.alamat">
                                            {{ item.alamat || '-' }}
                                        </div>
                                    </td>

                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span :class="getSiswaStatusDetail(item).color"
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium">
                                            {{ getSiswaStatusDetail(item).label }}
                                        </span>
                                    </td>

                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end gap-2">
                                            <Link :href="route('admin.siswa.show', item.id)"
                                            ...
                                            <Link :href="route('admin.siswa.edit', item.id)"
                    ...
                    <!-- Mobile Card View -->
                    <div v-if="siswa.data.length > 0" class="lg:hidden divide-y divide-gray-100 dark:divide-gray-800">
                        <div v-for="item in siswa.data" :key="item.id" class="p-4">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">NIS: {{ item.nis }}</p>
                                </div>
                            </div>
                            ...
                            <div class="mt-3 grid grid-cols-2 gap-2">
                                ...
                                <div class="rounded-lg bg-gray-50 dark:bg-gray-800/50 px-3 py-2">
                                    ...
                                    <p class="text-sm font-medium text-gray-900 dark:text-white mt-0.5">{{ item.angkatan }}</p>
                                </div>
                                <div class="col-span-2 rounded-lg bg-gray-50 dark:bg-gray-800/50 px-3 py-2">
                                    ...
                                    <p class="text-sm font-medium text-gray-900 dark:text-white mt-0.5">{{ item.alamat || '-' }}</p>
                                </div>
                            </div>
                    ...
                    <div v-else class="p-8 text-center sm:p-16">
                        <div class="mx-auto max-w-sm">
                            ...
                            <h3 class="mt-4 text-lg font-semibold text-gray-900 dark:text-white sm:mt-6">Belum ada data siswa</h3>
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                                {{ hasActiveFilters ? 'Coba sesuaikan kriteria pencarian atau bersihkan filter.' : 'Mulai dengan menambahkan siswa pertama.' }}
                            </p>
                            <div class="mt-6 sm:mt-8">
                                <Link :href="route('admin.siswa.create')"
                                    ...
                                    {{ hasActiveFilters ? 'Tambah Siswa Baru' : 'Tambah Siswa Pertama' }}
                                </Link>
                            </div>
                        </div>
                    </div>
                ...
                <div v-if="siswa.last_page > 1"
                    ...
                    <div class="text-sm text-gray-600 dark:text-gray-400">
                        Menampilkan
                        <span class="font-medium text-gray-900 dark:text-white">{{ ((siswa.current_page - 1) * siswa.per_page) + 1 }}</span>
                        sampai
                        <span class="font-medium text-gray-900 dark:text-white">{{ Math.min(siswa.current_page * siswa.per_page, siswa.total) }}</span>
                        dari
                        <span class="font-medium text-gray-900 dark:text-white">{{ siswa.total }}</span>
                        hasil
                    </div>
                    <nav class="flex flex-wrap items-center justify-center gap-1 sm:justify-start" aria-label="Navigasi halaman">
                        <template v-for="link in siswa.links" :key="link.label">
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto" @click.self="showDeleteModal = false">
            ...
            <div class="relative mx-4 w-full max-w-md transform rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-black/5 transition-all dark:bg-gray-900 dark:ring-white/10">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Hapus Siswa</h3>
                ...
                <div class="mt-4">
                    <p class="text-sm text-gray-700 dark:text-gray-300">
                        ...
                        <span class="font-semibold">"{{ selectedSiswa?.nama }}"</span> dengan NIS
                        <span class="font-semibold">{{ selectedSiswa?.nis }}</span>?
                    </p>
                </div>
                ...
                <div class="relative mx-4 pointer-events-auto">
                    <div class="rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-black/5 dark:bg-gray-900 dark:ring-white/10 max-w-sm">
                        ...
                        <div class="mt-4 text-center">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Siswa Berhasil Dihapus!</h3>
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Data siswa telah dihapus secara permanen dari sistem.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-if="showSuccessCreatePopup" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto">
                ...
                <div class="relative mx-4 pointer-events-auto">
                    <div class="rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-black/5 dark:bg-gray-900 dark:ring-white/10 max-w-sm">
                        ...
                        <div class="mt-4 text-center">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Siswa Berhasil Ditambahkan!</h3>
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Data siswa baru telah berhasil ditambahkan ke sistem.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-if="showSuccessUpdatePopup" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto">
                ...
                <div class="relative mx-4 pointer-events-auto">
                    <div class="rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-black/5 dark:bg-gray-900 dark:ring-white/10 max-w-sm">
                        ...
                        <div class="mt-4 text-center">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Data Siswa Berhasil Diperbarui!</h3>
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Perubahan data siswa telah berhasil disimpan.</p>
                        </div>
                    </div>
                </div>
            </div>
