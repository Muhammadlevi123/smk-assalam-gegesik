...
interface Siswa {
    ...
    angkatan:      string;
    foto?:         string;
    kelas: Array<{ id: number; nama_kelas: string; pivot: { siswa_id: number; kelas_id: number; tahun_ajaran_id: number } }>;
    tahunAjaranStatus: Array<{ id: number; tahun: string; pivot: { siswa_id: number; tahun_ajaran_id: number; status: string; kelulusan?: string | null } }>;
    kelas_form_data?:  Array<{ kelas_id: number; tahun_ajaran_id: number }>;
    status_form_data?: Array<{ tahun_ajaran_id: number; status: string; kelulusan?: string | null }>;
}
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit Siswa', href: `/admin/siswa/${props.siswa?.id}/edit` },
];
...
const prepareInitialKelasData = () => {
    if (props.siswa?.kelas_form_data?.length) {
        return props.siswa.kelas_form_data.map(item => ({
            kelas_id:        item.kelas_id.toString(),
            tahun_ajaran_id: item.tahun_ajaran_id.toString(),
        }));
    }
    if (!props.siswa?.kelas?.length) return [{ kelas_id: '', tahun_ajaran_id: '' }];
    return props.siswa.kelas.map(k => ({
        kelas_id:        k.id.toString(),
        tahun_ajaran_id: k.pivot.tahun_ajaran_id.toString(),
    }));
};

const prepareInitialStatusData = () => {
    if (props.siswa?.status_form_data?.length) {
        return props.siswa.status_form_data.map(item => ({
            tahun_ajaran_id: item.tahun_ajaran_id.toString(),
            status:          item.status,
            kelulusan:       item.kelulusan ?? '',
        }));
    }
    if (!props.siswa?.tahunAjaranStatus?.length) return [{ tahun_ajaran_id: '', status: 'Aktif', kelulusan: '' }];
    return props.siswa.tahunAjaranStatus.map(ta => ({
        tahun_ajaran_id: ta.id.toString(),
...
const formState = reactive({
    ...
    kelas_tahun_ajaran:  prepareInitialKelasData(),
...
// ── Kelulusan: hanya di baris TERAKHIR jika total >= 3 ────────────
const totalBaris        = computed(() => formState.status_tahun_ajaran.length);
...
const resetKelulusanJikaPerlu = () => {
    if (!bolehAdaKelulusan.value) {
        formState.status_tahun_ajaran.forEach(s => { s.kelulusan = ''; });
    }
};

// ── Status helpers ─────────────────────────────────────────────────
const addStatusTahunAjaran = () => {
    formState.status_tahun_ajaran.push({ tahun_ajaran_id: '', status: 'Aktif', kelulusan: '' });
};

const removeStatusTahunAjaran = (index: number) => {
    if (formState.status_tahun_ajaran.length <= 1) return;
    const removedId     = formState.status_tahun_ajaran[index].tahun_ajaran_id;
    const removedStatus = formState.status_tahun_ajaran[index].status;
    formState.status_tahun_ajaran.splice(index, 1);
    if (removedStatus === 'Aktif' && removedId) {
        ...
        if (!masihAktif) {
            formState.kelas_tahun_ajaran.forEach(c => { if (c.tahun_ajaran_id === removedId) c.tahun_ajaran_id = ''; });
        }
    }
...
const onStatusTahunChange = (index: number, oldId: string) => {
    ...
    if (!masihAktif) {
        formState.kelas_tahun_ajaran.forEach(c => { if (c.tahun_ajaran_id === oldId) c.tahun_ajaran_id = ''; });
    }
};
...
const onStatusValueChange = (index: number) => {
    ...
    if (item.status !== 'Aktif' && item.tahun_ajaran_id) {
        ...
        if (!masihAktif) {
            formState.kelas_tahun_ajaran.forEach(c => { if (c.tahun_ajaran_id === id) c.tahun_ajaran_id = ''; });
        }
    }
};

const getAvailableTahunAjaranForStatus = (currentIndex: number) => {
    const usedIds = formState.status_tahun_ajaran.filter((_, i) => i !== currentIndex).map(s => s.tahun_ajaran_id).filter(Boolean);
    return props.tahunAjaran.filter(t => !usedIds.includes(t.id.toString()));
};

// ── Kelas helpers ──────────────────────────────────────────────────
const addKelasKombinasi    = () => formState.kelas_tahun_ajaran.push({ kelas_id: '', tahun_ajaran_id: '' });
const removeKelasKombinasi = (index: number) => { if (formState.kelas_tahun_ajaran.length > 1) formState.kelas_tahun_ajaran.splice(index, 1); };

// Saat tahun ajaran kelas berubah, reset kelas_id
const onKelasRowTahunChange = (index: number) => {
    formState.kelas_tahun_ajaran[index].kelas_id = '';
...
const getAvailableTahunAjaranForKelas = (currentIndex: number) => {
    const usedIds = formState.kelas_tahun_ajaran.filter((_, i) => i !== currentIndex).map(c => c.tahun_ajaran_id).filter(Boolean);
    return tahunAjaranAktif.value.filter(t => !usedIds.includes(t.id.toString()));
};

const getAvailableKelas = (currentIndex: number) => {
    const usedIds = formState.kelas_tahun_ajaran.filter((_, i) => i !== currentIndex).map(c => c.kelas_id).filter(Boolean);
...
const getCurrentImageUrl = () => props.siswa?.foto ? `/storage/${props.siswa.foto}` : '/images/default-avatar.png';
...
const hasChanges = computed(() => {
    if (!props.siswa) return false;
    if (
        formState.nis           !== props.siswa.nis ||
        formState.nama          !== props.siswa.nama ||
        formState.jenis_kelamin !== props.siswa.jenis_kelamin ||
        formState.alamat        !== (props.siswa.alamat || '') ||
        formState.angkatan      !== props.siswa.angkatan ||
        formState.foto          !== null
    ) return true;

    const initKelas = prepareInitialKelasData();
    if (formState.kelas_tahun_ajaran.length !== initKelas.length) return true;
    for (let i = 0; i < formState.kelas_tahun_ajaran.length; i++) {
        if (
            formState.kelas_tahun_ajaran[i].kelas_id        !== initKelas[i]?.kelas_id ||
            formState.kelas_tahun_ajaran[i].tahun_ajaran_id !== initKelas[i]?.tahun_ajaran_id
        ) return true;
    }
    ...
    for (let i = 0; i < formState.status_tahun_ajaran.length; i++) {
        if (
            formState.status_tahun_ajaran[i].tahun_ajaran_id !== initStatus[i]?.tahun_ajaran_id ||
            formState.status_tahun_ajaran[i].status          !== initStatus[i]?.status ||
            formState.status_tahun_ajaran[i].kelulusan       !== (initStatus[i]?.kelulusan ?? '')
        ) return true;
    }
    return false;
});
...
const handleSubmit = async () => {
    ...
    if (!formState.nis.trim())      { setError('nis', 'NIS wajib diisi'); trySetFirst(refNis.value); }
    if (!formState.nama.trim())     { setError('nama', 'Nama lengkap wajib diisi'); trySetFirst(refNama.value); }
    if (!formState.jenis_kelamin)   { setError('jenis_kelamin', 'Jenis kelamin wajib dipilih'); trySetFirst(refJenisKelamin.value); }
    if (!formState.angkatan.trim()) { setError('angkatan', 'Angkatan wajib diisi'); trySetFirst(refAngkatan.value); }
    ...
    for (let i = 0; i < formState.kelas_tahun_ajaran.length; i++) {
        if (!formState.kelas_tahun_ajaran[i].tahun_ajaran_id) { setError(`kelas_${i}_tahun`, 'Tahun ajaran wajib dipilih'); trySetFirst(refKelasRows.value[i] ?? null); }
        if (!formState.kelas_tahun_ajaran[i].kelas_id)        { setError(`kelas_${i}_kelas`, 'Kelas wajib dipilih'); trySetFirst(refKelasRows.value[i] ?? null); }
    }

    const kelasCombos = formState.kelas_tahun_ajaran.map(c => `${c.kelas_id}-${c.tahun_ajaran_id}`);
    ...
    fd.append('nis',           formState.nis);
    fd.append('nama',          formState.nama);
    fd.append('jenis_kelamin', formState.jenis_kelamin);
    fd.append('alamat',        formState.alamat || '');
    fd.append('angkatan',      formState.angkatan);
    if (formState.foto instanceof File) fd.append('foto', formState.foto);

    formState.kelas_tahun_ajaran.forEach((c, i) => {
        fd.append(`kelas_tahun_ajaran[${i}][kelas_id]`,        c.kelas_id);
        fd.append(`kelas_tahun_ajaran[${i}][tahun_ajaran_id]`, c.tahun_ajaran_id);
    });
    ...
    formState.status_tahun_ajaran.forEach((s, i) => {
        ...
        fd.append(`status_tahun_ajaran[${i}][kelulusan]`,
            isBarisKelulusan(i) ? (s.kelulusan || '') : '');
    });

    processing.value = true;

    router.post(`/admin/siswa/${props.siswa.id}`, fd, {
        preserveScroll: true,
        onError: (errs) => {
...
<template>
    <Head :title="`Edit Siswa - ${siswa?.nama || 'Loading...'}`" />
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="bg-gray-50/50 dark:bg-gray-950/50 min-h-screen">
            <div class="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">

                <div class="space-y-2">
                    <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Edit Siswa</h1>
                    <p class="text-base text-gray-600 dark:text-gray-400">Perbarui informasi untuk siswa "{{ siswa?.nama }}"</p>
                </div>
                    ...
                    <div class="xl:col-span-1">
                        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900 h-fit">
                            <div class="border-b border-gray-100 bg-gray-50/50 px-6 py-4 dark:border-gray-800 dark:bg-gray-800/50">
                                ...
                                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Perbarui foto profil siswa (opsional)</p>
                            </div>
                            <div class="p-6 space-y-4">
                                <div class="flex justify-center">
                                    <div class="relative">
                                        <img :src="imagePreview || getCurrentImageUrl()" :alt="siswa?.nama"
                                        ...
                                        <button v-if="imagePreview" @click="removeImage" type="button"
                                            class="absolute -right-2 -top-2 rounded-full bg-red-100 p-2 text-red-600 hover:bg-red-200 dark:bg-red-900/50 dark:text-red-400">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                                        </button>
                                        <div v-if="imagePreview" class="absolute -bottom-2 left-1/2 -translate-x-1/2">
                                ...
                                <button @click="fileInputRef?.click()" type="button"
                                    ...
                                    {{ imagePreview ? 'Ganti Foto' : 'Perbarui Foto' }}
                                </button>
                    ...
                    <div class="xl:col-span-2">
                        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                            <div class="border-b border-gray-100 bg-gray-50/50 px-6 py-4 dark:border-gray-800 dark:bg-gray-800/50">
                                ...
                                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Perbarui data siswa dengan kelas dan status per tahun ajaran.</p>
                            </div>
                            ...
                            <form @submit.prevent="handleSubmit" class="p-6">
                                <div class="space-y-8">
                                    ...
                                    <div class="rounded-xl border border-gray-200 bg-gray-50/50 p-5 dark:border-gray-700 dark:bg-gray-800/50">
                                        <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Data Siswa Saat Ini</h4>
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div class="min-w-0 flex-1">
                                                <div class="text-base font-medium text-gray-900 dark:text-white">{{ siswa.nama }}</div>
                                                <div class="text-sm text-gray-600 dark:text-gray-400">NIS: {{ siswa.nis }} · {{ siswa.jenis_kelamin }}</div>
                                                <div class="text-sm text-gray-500 dark:text-gray-400">Angkatan {{ siswa.angkatan }}</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div ref="refNis" class="space-y-1.5">
                                            ...
                                            <input v-model="formState.nis" @input="delete localErrors['nis']" type="text" placeholder="Masukkan NIS siswa"
                                            ...
                                            <p v-if="localErrors.nis || serverErrors.nis" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1">
                                                <span v-html="ErrIcon()"></span>{{ localErrors.nis || serverErrors.nis }}
                                            </p>
                                        </div>
                                        ...
                                        <div ref="refNama" class="space-y-1.5">
                                            ...
                                            <input v-model="formState.nama" @input="delete localErrors['nama']" type="text" placeholder="Masukkan nama lengkap siswa"
                                        ...
                                        <div ref="refAngkatan" class="space-y-1.5">
                                            ...
                                            <input v-model="formState.angkatan" @input="delete localErrors['angkatan']" type="text" :placeholder="currentYear.toString()"
                                            ...
                                            <p v-if="localErrors.angkatan || serverErrors.angkatan" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1">
                                                <span v-html="ErrIcon()"></span>{{ localErrors.angkatan || serverErrors.angkatan }}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="space-y-1.5">
                                        ...
                                        <textarea v-model="formState.alamat" rows="3" placeholder="Masukkan alamat lengkap siswa"
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <div class="space-y-3">
                                            <div v-for="(statusItem, index) in formState.status_tahun_ajaran" :key="index"
                                                ...
                                                <div v-if="formState.status_tahun_ajaran.length > 1" class="mt-3 flex justify-end">
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <div v-else class="space-y-3">
                                            <div v-for="(combo, index) in formState.kelas_tahun_ajaran" :key="index"
                                                ...
                                                <div class="flex items-start pt-5">
                                                    <button v-if="formState.kelas_tahun_ajaran.length > 1" @click="removeKelasKombinasi(index)" type="button"
                                        ...
                                        <p v-if="serverErrors.kelas_tahun_ajaran" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1">
                                            <span v-html="ErrIcon()"></span>{{ serverErrors.kelas_tahun_ajaran }}
                                        </p>
                                    </div>
                                ...
                                <div class="mt-8 flex items-center gap-4 border-t border-gray-100 pt-6 dark:border-gray-800">
                                    <button type="submit" :disabled="processing || !hasChanges"
                                        ...
                                        {{ processing ? 'Memperbarui Siswa...' : 'Perbarui Siswa' }}
                                    </button>
