...
const jurusanOptions = [
    'IPA','IPS','Teknik Komputer dan Jaringan','Rekayasa Perangkat Lunak','Multimedia',
    'Akuntansi','Administrasi Perkantoran','Pemasaran','Teknik Mesin','Teknik Elektro',
    'Teknik Sipil','Teknik Kendaraan Ringan Otomotif','Teknik Otomotif',
];
...
const buildInitialWali = () => {
    ...
    // ✅ Default 1 row kosong jika tidak ada data, sama seperti guru
...
const buildInitialSiswaPerTahun = () => {
    ...
    // ✅ Default 1 row kosong jika tidak ada data
...
// ── Accordion: buka semua yang sudah ada data ─────────────────────
...
// ── Wali helpers ──────────────────────────────────────────────────
const addWaliKelas = () => {
    formState.wali_kelas.push({ guru_id: '', tahun_ajaran_id: '' });
    expandedWali.value.push(formState.wali_kelas.length - 1);
};

// ✅ Pola guru: reset ke kosong jika hanya 1 row, hapus jika lebih
const removeWaliKelas = (i: number) => {
    if (formState.wali_kelas.length > 1) {
        formState.wali_kelas.splice(i, 1);
        expandedWali.value = expandedWali.value.filter(n => n !== i).map(n => n > i ? n - 1 : n);
    } else {
        formState.wali_kelas[0] = { guru_id: '', tahun_ajaran_id: '' };
    }
};
...
// ── Siswa helpers ─────────────────────────────────────────────────
const addSiswaGroup = () => {
    formState.siswa_per_tahun.push({ tahun_ajaran_id: '', siswa_ids: [] });
    siswaSearch.value.push('');
    expandedSiswa.value.push(formState.siswa_per_tahun.length - 1);
};

// ✅ Pola guru: reset ke kosong jika hanya 1 row, hapus jika lebih
const removeSiswaGroup = (i: number) => {
    if (formState.siswa_per_tahun.length > 1) {
        formState.siswa_per_tahun.splice(i, 1);
        siswaSearch.value.splice(i, 1);
        expandedSiswa.value = expandedSiswa.value.filter(n => n !== i).map(n => n > i ? n - 1 : n);
    } else {
        formState.siswa_per_tahun[0] = { tahun_ajaran_id: '', siswa_ids: [] };
        siswaSearch.value[0] = '';
    }
};
...
const onSiswaGroupTahunChange = (index: number) => {
    formState.siswa_per_tahun[index].siswa_ids = [];
    setSiswaSearch(index, '');
};
...
const hasChanges = computed(() => {
    ...
    const currentFlat = formState.siswa_per_tahun
        .flatMap(g => g.siswa_ids.map(id => `${g.tahun_ajaran_id}:${id}`))
        .sort().join(',');
    const originalFlat = props.kelas.siswa_kelas_form
        .filter(s => s.siswa_id && s.tahun_ajaran_id)
        .map(s => `${s.tahun_ajaran_id}:${s.siswa_id}`)
        .sort().join(',');
...
const handleSubmit = async () => {
    ...
    // ✅ Validasi wali: skip row yang keduanya kosong
    formState.wali_kelas.forEach((w, i) => {
        ...
        if (w.guru_id && w.tahun_ajaran_id && isDupWali(w.guru_id, w.tahun_ajaran_id, i)) {
            setError(`wali_${i}_dup`, 'Kombinasi sudah ada'); trySet(refWaliRows.value[i]); if (!isWaliExpanded(i)) expandedWali.value.push(i);
        }
    });

    // ✅ Validasi siswa: skip row yang tahunnya kosong
    const usedTahun: string[] = [];
    formState.siswa_per_tahun.forEach((g, i) => {
        if (!g.tahun_ajaran_id) return;
        if (usedTahun.includes(g.tahun_ajaran_id)) {
            setError(`siswa_${i}_tahun`, 'Tahun ajaran sudah dipakai di grup lain'); trySet(refSiswaRows.value[i]); if (!isSiswaExpanded(i)) expandedSiswa.value.push(i);
        }
    ...
    // ✅ Hanya kirim siswa yang tahunnya terisi dan ada siswa dipilih
    let fi = 0;
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                                ...
                                <div ref="refJurusan" class="space-y-1.5">
                                    ...
                                    <input v-model="formState.jurusan" @input="delete localErrors['jurusan']" list="jurusan-list" placeholder="Pilih atau ketik jurusan"
                                    ...
                                    <datalist id="jurusan-list">
                                        <option v-for="j in jurusanOptions" :key="j" :value="j" />
                                        <option v-for="j in jurusanList" :key="j.nama" :value="j.nama" />
                                    </datalist>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Pilih atau ketik jurusan baru</p>
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Pilih tahun ajaran, lalu centang siswa. Hanya siswa berstatus <span class="font-medium text-green-700 dark:text-green-400">Aktif</span> yang tersedia. Biarkan kosong jika tidak ada.</p>
                                    </div>
