...
interface Kelas {
    ...
    wali_kelas_form:  Array<{ guru_id: string; tahun_ajaran_id: string }>;
    siswa_kelas_form: Array<{ siswa_id: string; tahun_ajaran_id: string }>;
}
// ✅ jurusanList format {value, label} — sama dengan Create
interface Props {
    kelas:       Kelas;
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: `Edit ${props.kelas.nama_kelas}`, href: `/admin/kelas/${props.kelas.id}/edit` },
];
...
const buildInitialWali = () => {
    const w = props.kelas.wali_kelas_form
        .filter(w => w.guru_id && w.tahun_ajaran_id)
        .map(w => ({ guru_id: w.guru_id.toString(), tahun_ajaran_id: w.tahun_ajaran_id.toString() }));
    return w.length > 0 ? w : [{ guru_id: '', tahun_ajaran_id: '' }];
};
const buildInitialSiswaPerTahun = () => {
    const map: Record<string, string[]> = {};
    for (const item of props.kelas.siswa_kelas_form) {
        if (!item.siswa_id || !item.tahun_ajaran_id) continue;
        if (!map[item.tahun_ajaran_id]) map[item.tahun_ajaran_id] = [];
        map[item.tahun_ajaran_id].push(item.siswa_id);
    }
    const result = Object.entries(map).map(([taId, ids]) => ({ tahun_ajaran_id: taId, siswa_ids: ids }));
    return result.length > 0 ? result : [{ tahun_ajaran_id: '', siswa_ids: [] as string[] }];
};
...
const formState = reactive({
    ...
    siswa_per_tahun: buildInitialSiswaPerTahun(),
});
...
const onJurusanInput = () => { jurusanSearch.value = formState.jurusan; showJurusanDropdown.value = true; delete localErrors.value['jurusan']; };
const onJurusanFocus = () => { jurusanSearch.value = formState.jurusan; showJurusanDropdown.value = true; };
const selectJurusan  = (val: string) => { formState.jurusan = val; jurusanSearch.value = val; showJurusanDropdown.value = false; delete localErrors.value['jurusan']; };
const closeJurusanDropdown = () => { showJurusanDropdown.value = false; };

// ── Accordion ─────────────────────────────────────────────────────
const expandedWali  = ref<number[]>(formState.wali_kelas.map((_, i) => i));
const expandedSiswa = ref<number[]>(formState.siswa_per_tahun.map((_, i) => i));
...
const siswaSearch    = ref<string[]>(formState.siswa_per_tahun.map(() => ''));
...
const waliAccordionLabel = (index: number) => {
    ...
    return `${tahun} — ${props.guru.find(g => g.id.toString() === w.guru_id)?.nama ?? ''}`;
};
...
const getAvailableSiswaForGroup = (taId: string, currentIndex: number) => {
    ...
    return props.siswa.filter(s => {
        return s.tahun_ajaran_aktif.includes(id) &&
            !s.existing_kelas_tahun.some(e => e.tahun_ajaran_id === id) &&
            !formState.siswa_per_tahun.some((g, i) => i !== currentIndex && g.siswa_ids.includes(s.id.toString()));
    });
};
...
const siswaAccordionLabel = (index: number) => {
    ...
    return g.siswa_ids.length === 0 ? tahun : `${tahun} — ${g.siswa_ids.length} siswa dipilih`;
};

// ── hasChanges ────────────────────────────────────────────────────
const hasChanges = computed(() => {
    if (formState.nama_kelas !== props.kelas.nama_kelas ||
        formState.jurusan    !== props.kelas.jurusan    ||
        formState.tingkat    !== props.kelas.tingkat)   return true;
    const initWali = buildInitialWali();
    if (formState.wali_kelas.length !== initWali.length) return true;
    for (let i = 0; i < formState.wali_kelas.length; i++) {
        if (formState.wali_kelas[i].guru_id         !== (initWali[i]?.guru_id         ?? '') ||
            formState.wali_kelas[i].tahun_ajaran_id !== (initWali[i]?.tahun_ajaran_id ?? '')) return true;
    }
    const currentFlat  = formState.siswa_per_tahun.flatMap(g => g.siswa_ids.map(id => `${g.tahun_ajaran_id}:${id}`)).sort().join(',');
    const originalFlat = props.kelas.siswa_kelas_form.filter(s => s.siswa_id && s.tahun_ajaran_id).map(s => `${s.tahun_ajaran_id}:${s.siswa_id}`).sort().join(',');
    return currentFlat !== originalFlat;
});
...
const handleSubmit = async () => {
    ...
    formState.wali_kelas.forEach((w, i) => {
        const isEmpty = !w.guru_id && !w.tahun_ajaran_id; if (isEmpty) return;
        ...
        if (w.guru_id && w.tahun_ajaran_id && isDupWali(w.guru_id, w.tahun_ajaran_id, i)) { setError(`wali_${i}_dup`, 'Kombinasi sudah ada'); trySet(refWaliRows.value[i]); if (!isWaliExpanded(i)) expandedWali.value.push(i); }
    });
    const usedTahun: string[] = [];
    formState.siswa_per_tahun.forEach((g, i) => {
        if (!g.tahun_ajaran_id) return;
        if (usedTahun.includes(g.tahun_ajaran_id)) { setError(`siswa_${i}_tahun`, 'Tahun ajaran sudah dipakai'); trySet(refSiswaRows.value[i]); if (!isSiswaExpanded(i)) expandedSiswa.value.push(i); }
    ...
    formState.siswa_per_tahun.forEach(g => {
        if (g.tahun_ajaran_id && g.siswa_ids.length > 0) {
            g.siswa_ids.forEach(sid => { fd.append(`siswa_kelas[${fi}][siswa_id]`, sid); fd.append(`siswa_kelas[${fi}][tahun_ajaran_id]`, g.tahun_ajaran_id); fi++; });
        }
    });

    processing.value = true;
    router.post(`/admin/kelas/${props.kelas.id}`, fd, {
        preserveScroll: true,
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="bg-gray-50/50 dark:bg-gray-950/50 min-h-screen">
            <div class="mx-auto max-w-4xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
                ...
                <div class="space-y-2">
                    <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Edit Kelas</h1>
                    <p class="text-base text-gray-600 dark:text-gray-400">Perbarui informasi untuk kelas "{{ kelas.nama_kelas }}"</p>
                </div>
                ...
                <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <div class="border-b border-gray-100 bg-gray-50/50 px-6 py-4 dark:border-gray-800 dark:bg-gray-800/50">
                        ...
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Perbarui data kelas beserta wali kelas dan daftar siswa</p>
                    </div>
                    ...
                    <form @submit.prevent="handleSubmit" class="p-6">
                        <div class="space-y-8">
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Pilih tahun ajaran dulu, guru akan difilter otomatis.</p>
                                    </div>
                                    <button @click="addWaliKelas" type="button" class="inline-flex items-center gap-1.5 rounded-lg bg-green-50 px-3 py-2 text-sm font-medium text-green-700 hover:bg-green-100 dark:bg-green-900/20 dark:text-green-400 dark:hover:bg-green-900/40">
                                ...
                                <div class="space-y-2">
                                    <div v-for="(wali, index) in formState.wali_kelas" :key="index"
                                        ...
                                        <button type="button" @click="toggleWaliExpand(index)" class="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-green-50 dark:hover:bg-green-900/10 transition-colors">
                                            <div class="flex items-center gap-2 min-w-0">
                                                ...
                                                <span v-if="localErrors[`wali_${index}_tahun`] || localErrors[`wali_${index}_guru`] || localErrors[`wali_${index}_dup`]" class="flex-shrink-0 inline-flex items-center gap-1 rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-700 dark:bg-red-900/30 dark:text-red-400"><span class="h-1.5 w-1.5 rounded-full bg-red-500"></span>Ada error</span>
                                            </div>
                                            <div class="flex items-center gap-2 flex-shrink-0 ml-2">
                                                <button @click.stop="removeWaliKelas(index)" type="button" class="inline-flex items-center justify-center rounded-lg p-1.5 text-red-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
                                        ...
                                        <div v-show="isWaliExpanded(index)" class="border-t border-green-100 bg-green-50/50 px-4 py-4 dark:border-green-900 dark:bg-green-900/5">
                                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                                <div class="space-y-1.5">
                                                    ...
                                                    <div class="relative">
                                                        <select v-model="wali.tahun_ajaran_id" @change="onWaliTahunChange(index); delete localErrors[`wali_${index}_tahun`]" :class="localErrors[`wali_${index}_tahun`] ? 'ring-red-400' : 'ring-gray-200'" class="block w-full appearance-none rounded-lg border-0 bg-white py-2.5 pl-3 pr-8 text-sm text-gray-900 ring-1 ring-inset focus:ring-2 focus:ring-green-600 dark:bg-gray-700 dark:text-white dark:ring-gray-600">
                                                ...
                                                <div class="space-y-1.5">
                                                    ...
                                                    <div class="relative">
                                                        <select v-model="wali.guru_id" :disabled="!wali.tahun_ajaran_id" @change="delete localErrors[`wali_${index}_guru`]" :class="localErrors[`wali_${index}_guru`] || localErrors[`wali_${index}_dup`] ? 'ring-red-400' : 'ring-gray-200'" class="block w-full appearance-none rounded-lg border-0 bg-white py-2.5 pl-3 pr-8 text-sm text-gray-900 ring-1 ring-inset focus:ring-2 focus:ring-green-600 dark:bg-gray-700 dark:text-white dark:ring-gray-600 disabled:bg-gray-100 disabled:cursor-not-allowed dark:disabled:bg-gray-800">
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center justify-between">
                                    ...
                                    <button @click="addSiswaGroup" type="button" class="inline-flex items-center gap-1.5 rounded-lg bg-purple-50 px-3 py-2 text-sm font-medium text-purple-700 hover:bg-purple-100 dark:bg-purple-900/20 dark:text-purple-400 dark:hover:bg-purple-900/40">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>Tambah
                                    </button>
                                </div>
                                <div class="space-y-2">
                                    <div v-for="(group, index) in formState.siswa_per_tahun" :key="index"
                                        ...
                                        <button type="button" @click="toggleSiswaExpand(index)" class="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-purple-50 dark:hover:bg-purple-900/10 transition-colors">
                                            <div class="flex items-center gap-2 min-w-0">
                                                ...
                                                <span v-if="localErrors[`siswa_${index}_tahun`]" class="flex-shrink-0 inline-flex items-center gap-1 rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-700 dark:bg-red-900/30 dark:text-red-400"><span class="h-1.5 w-1.5 rounded-full bg-red-500"></span>Ada error</span>
                                            </div>
                                            <div class="flex items-center gap-2 flex-shrink-0 ml-2">
                                                <button @click.stop="removeSiswaGroup(index)" type="button" class="inline-flex items-center justify-center rounded-lg p-1.5 text-red-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
                                        ...
                                        <div v-show="isSiswaExpanded(index)" class="border-t border-purple-100 bg-purple-50/50 px-4 py-4 dark:border-purple-900 dark:bg-purple-900/5 space-y-3">
                                            <div class="space-y-1.5">
                                                ...
                                                <div class="relative w-full sm:w-56">
                                                    <select v-model="group.tahun_ajaran_id" @change="onSiswaGroupTahunChange(index); delete localErrors[`siswa_${index}_tahun`]" :class="localErrors[`siswa_${index}_tahun`] ? 'ring-red-400' : 'ring-gray-200'" class="block w-full appearance-none rounded-lg border-0 bg-white py-2.5 pl-3 pr-8 text-sm text-gray-900 ring-1 ring-inset focus:ring-2 focus:ring-purple-600 dark:bg-gray-700 dark:text-white dark:ring-gray-600">
                                                        <option value="">Pilih tahun ajaran</option>
                                            ...
                                            <template v-if="group.tahun_ajaran_id">
                                                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                                    <div class="relative flex-1">
                                                        ...
                                                        <input :value="getSiswaSearch(index)" @input="setSiswaSearch(index, ($event.target as HTMLInputElement).value)" type="text" placeholder="Cari nama atau NIS siswa..." class="block w-full rounded-lg border-0 bg-white py-2 pl-9 pr-3 text-sm text-gray-900 ring-1 ring-inset ring-gray-200 placeholder:text-gray-400 focus:ring-2 focus:ring-purple-600 dark:bg-gray-700 dark:text-white dark:ring-gray-600 dark:placeholder:text-gray-500" />
                                                    </div>
                                                ...
                                                <p class="text-xs text-gray-500 dark:text-gray-400">{{ getAvailableSiswaForGroup(group.tahun_ajaran_id, index).length }} siswa aktif tersedia<span v-if="getSiswaSearch(index)"> · {{ getFilteredSiswa(group.tahun_ajaran_id, index).length }} hasil pencarian</span></p>
                                                <div v-if="getFilteredSiswa(group.tahun_ajaran_id, index).length > 0" class="max-h-60 overflow-y-auto rounded-lg border border-purple-200 bg-white dark:border-purple-800 dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
                                                    <label v-for="s in getFilteredSiswa(group.tahun_ajaran_id, index)" :key="s.id" class="flex items-center gap-3 px-4 py-2.5 cursor-pointer hover:bg-purple-50 dark:hover:bg-purple-900/10 transition-colors" :class="isSiswaSelected(index, s.id.toString()) ? 'bg-purple-50 dark:bg-purple-900/20' : ''">
                                                        <input type="checkbox" :checked="isSiswaSelected(index, s.id.toString())" @change="toggleSiswa(index, s.id.toString())" class="h-4 w-4 flex-shrink-0 rounded border-gray-300 text-purple-600 focus:ring-purple-600 dark:border-gray-600 dark:bg-gray-700" />
                                                        <div class="flex items-center gap-2 min-w-0 flex-1">
                                                            ...
                                                            <div class="min-w-0"><p class="text-sm font-medium text-gray-900 dark:text-white truncate">{{ s.nama }}</p><p class="text-xs text-gray-500 dark:text-gray-400">{{ s.nis }} · {{ s.jenis_kelamin }}</p></div>
                                                        </div>
                                                    </label>
                                                </div>
                                                <div v-else-if="getSiswaSearch(index)" class="rounded-lg border border-dashed border-purple-200 p-5 text-center dark:border-purple-800"><p class="text-sm text-gray-500 dark:text-gray-400">Tidak ada siswa yang cocok</p></div>
                                                <div v-else class="rounded-lg border border-dashed border-purple-200 p-5 text-center dark:border-purple-800"><p class="text-sm text-gray-500 dark:text-gray-400">Semua siswa aktif sudah terdaftar di kelas lain</p></div>
                                            </template>
                            ...
                            <!-- Indikator perubahan -->
                            <div v-if="hasChanges" class="rounded-xl bg-blue-50 border border-blue-200 p-3 dark:bg-blue-900/10 dark:border-blue-800">
                                <div class="flex items-center gap-2">
                                    <div class="h-2 w-2 rounded-full bg-blue-500 animate-pulse flex-shrink-0"></div>
                        ...
                        <div class="mt-8 flex flex-col gap-3 border-t border-gray-100 pt-6 dark:border-gray-800 sm:flex-row sm:items-center">
                            <button type="submit" :disabled="processing || !hasChanges"
                                ...
                                {{ processing ? 'Memperbarui Kelas...' : 'Perbarui Kelas' }}
                            </button>
