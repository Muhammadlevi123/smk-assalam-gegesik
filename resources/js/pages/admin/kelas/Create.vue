...
const formState = reactive({
    ...
    jurusan:         '',
    tingkat:         '',
    // ✅ Selalu ada 1 row kosong di awal — sama seperti guru
...
const handleSubmit = async () => {
    ...
    // ✅ Validasi wali: skip row yang keduanya kosong (opsional)
    formState.wali_kelas.forEach((w, i) => {
        const isEmpty = !w.guru_id && !w.tahun_ajaran_id;
        if (isEmpty) return; // row kosong = opsional, diabaikan
        ...
        if (w.guru_id && w.tahun_ajaran_id && isDupWali(w.guru_id, w.tahun_ajaran_id, i)) {
            setError(`wali_${i}_dup`, 'Kombinasi guru & tahun sudah ada'); trySet(refWaliRows.value[i]); if (!isWaliExpanded(i)) expandedWali.value.push(i);
        }
    });

    // ✅ Validasi siswa: skip row yang tahunnya kosong (opsional)
    const usedTahun: string[] = [];
    formState.siswa_per_tahun.forEach((g, i) => {
        if (!g.tahun_ajaran_id) return; // row kosong = opsional, diabaikan
    ...
    // ✅ Hanya kirim wali yang lengkap (keduanya terisi)
                                    ...
                                    <div v-for="(wali, index) in formState.wali_kelas" :key="index"
                                        ...
                                        <button type="button" @click="toggleWaliExpand(index)"
                                            ...
                                            <div class="flex items-center gap-2 flex-shrink-0 ml-2">
                                                <!-- ✅ Tombol X selalu tampil (reset ke kosong jika hanya 1 row) -->
