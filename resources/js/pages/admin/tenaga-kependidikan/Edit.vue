...
interface TenagaKependidikan {
    id: number; nama: string; jenis_kelamin: string; jabatan: string;
    alamat?: string; foto?: string;
...
const hapusFotoAction = () => {
    hapusFoto.value = true; existingFoto.value = null;
    formState.foto  = null; imagePreview.value = null;
    if (fileInputRef.value) fileInputRef.value.value = '';
};
...
const removeStatusTahunAjaran = (index: number) => { if (formState.status_tahun_ajaran.length > 1) formState.status_tahun_ajaran.splice(index, 1); };
...
const hasChanges = computed(() => {
    ...
    if (formState.nama         !== props.tenagaKependidikan.nama ||
        ...
        formState.foto !== null || hapusFoto.value) return true;
    const init = prepareInitialStatusData();
    if (formState.status_tahun_ajaran.length !== init.length) return true;
    for (let i = 0; i < formState.status_tahun_ajaran.length; i++) {
        if (formState.status_tahun_ajaran[i].tahun_ajaran_id !== init[i]?.tahun_ajaran_id ||
            formState.status_tahun_ajaran[i].status          !== init[i]?.status) return true;
    }
    return false;
});
...
const handleSubmit = async () => {
    ...
    if (!formState.jabatan.trim()) { setError('jabatan', 'Jabatan wajib diisi'); trySetFirst(refJabatan.value); }
...
<template>
    <Head :title="`Edit Tenaga Kependidikan - ${tenagaKependidikan?.nama || ''}`" />
                    ...
                    <div class="xl:col-span-1">
                        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900 h-fit">
                            ...
                            <div class="p-6 space-y-4">
                                <div class="flex justify-center">
                                    ...
                                    <div v-else>
                                        ...
                                        <div v-if="imagePreview" class="mt-2 flex justify-center">
                                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300">Foto Baru</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <button v-if="(tenagaKependidikan.foto && !hapusFoto) || imagePreview"
                                    @click="imagePreview ? removeImage() : hapusFotoAction()" type="button"
                                ...
                                <button v-if="hapusFoto && !imagePreview"
                                    @click="hapusFoto = false; existingFoto = tenagaKependidikan.foto ? `/storage/${tenagaKependidikan.foto}` : null"
                                    type="button"
                                ...
                                <div class="space-y-8">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div ref="refJabatan" class="space-y-1.5">
                                                ...
                                                <div v-if="showJabatanDropdown && filteredJabatan.length > 0"
                                                    ...
                                                    <div class="py-1 max-h-48 overflow-y-auto">
                                                        <button v-for="j in filteredJabatan" :key="j.value" type="button"
                                                            ...
                                                            :class="formState.jabatan === j.value ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-300' : ''">
                                                            {{ j.label }}
                                                        </button>
                                                    </div>
                                                    <div v-if="formState.jabatan && !filteredJabatan.find(j => j.value === formState.jabatan)"
                                                        class="border-t border-gray-100 dark:border-gray-700 px-4 py-2.5">
                                                        <p class="text-xs text-gray-500 dark:text-gray-400">Jabatan baru: <span class="font-semibold text-blue-600 dark:text-blue-400">"{{ formState.jabatan }}"</span></p>
                                                    </div>
                                                </div>
                                                <div v-if="showJabatanDropdown && filteredJabatan.length === 0 && formState.jabatan"
                                            ...
                                            <p v-if="localErrors.jabatan || serverErrors.jabatan" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1"><span v-html="ErrIcon()"></span>{{ localErrors.jabatan || serverErrors.jabatan }}</p>
                                        </div>
                                        ...
                                        <div ref="refJenisKelamin" class="space-y-1.5">
                                            ...
                                            <p v-if="localErrors.jenis_kelamin || serverErrors.jenis_kelamin" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1"><span v-html="ErrIcon()"></span>{{ localErrors.jenis_kelamin || serverErrors.jenis_kelamin }}</p>
                                        </div>
                                    </div>
