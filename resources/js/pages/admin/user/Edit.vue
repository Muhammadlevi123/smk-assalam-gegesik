    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="bg-gray-50/50 dark:bg-gray-950/50 min-h-screen">
            <div class="mx-auto max-w-4xl space-y-6 px-3 py-4 sm:px-4 sm:py-6 lg:px-8 lg:py-8">
                ...
                <!-- ══ Header ══ -->
                <div class="space-y-2">
                    <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-3xl">Edit User</h1>
                    <p class="text-sm text-gray-600 dark:text-gray-400 sm:text-base">Perbarui informasi akun dan hak akses pengguna</p>
                </div>
                ...
                <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900 sm:rounded-2xl">
                    <div class="border-b border-gray-100 bg-gray-50/50 px-4 py-3 dark:border-gray-800 dark:bg-gray-800/50 sm:px-6 sm:py-4">
                        <h3 class="text-sm font-medium text-gray-900 dark:text-white">Edit Informasi User</h3>
                    </div>

                    <form @submit.prevent="submit" class="p-4 sm:p-6 space-y-8">

                        <!-- Data Saat Ini -->
                        <div class="rounded-xl border border-gray-200 bg-gray-50/50 p-4 dark:border-gray-700 dark:bg-gray-800/50">
                            <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Data User Saat Ini</h4>
                            <div class="flex items-center gap-4">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <div class="flex items-center gap-2 mt-1">
                                        <span :class="getRoleColor(user.role)" class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium">
                                            {{ user.role.charAt(0).toUpperCase() + user.role.slice(1) }}
                                        </span>
                                        <span class="text-xs text-gray-400 dark:text-gray-500">Dibuat: {{ formatDate(user.created_at) }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="rounded-xl border border-amber-200 bg-amber-50/50 p-4 dark:border-amber-800 dark:bg-amber-900/20">
                                <div class="flex items-start gap-3">
                                    <div class="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg bg-amber-100 dark:bg-amber-900/40">
                            ...
                            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Minimal 8 karakter (opsional)</p>
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <div class="relative">
                                        <input id="password_confirmation" v-model="form.password_confirmation"
                                            ...
                                            class="block w-full rounded-xl border-0 bg-gray-50 py-3 pl-4 pr-11 text-sm text-gray-900 ring-1 ring-inset placeholder:text-gray:placeholder:text-gray-400 focus:bg-white focus:ring-2 dark:bg-gray-800 dark:text-white dark:ring-gray-700 dark:placeholder:text-gray-500 dark:focus:bg-gray-700" />
                                    ...
                                    <div v-for="roleOption in roles" :key="roleOption.value" class="relative">
                                        ...
                                        <label :for="`role-${roleOption.value}`"
                                            ...
                                            <div class="flex flex-1 items-center gap-3">
                                                <div class="h-4 w-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all"
                        ...
                        <!-- Indikator perubahan -->
                        <div v-if="hasChanges" class="rounded-xl border border-blue-200 bg-blue-50/50 p-4 dark:border-blue-800 dark:bg-blue-900/20">
                            <div class="flex items-center gap-2 mb-2">
                                <div class="h-2 w-2 rounded-full bg-blue-500 animate-pulse"></div>
                                <span class="text-sm font-medium text-blue-700 dark:text-blue-300">Ada perubahan yang belum disimpan</span>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1 text-sm text-blue-800 dark:text-blue-200">
                                <p v-if="form.name  !== user.name">  <span class="font-medium">Nama:</span>  {{ user.name }}  → {{ form.name }}</p>
                                <p v-if="form.email !== user.email"> <span class="font-medium">Email:</span> {{ user.email }} → {{ form.email }}</p>
                                <p v-if="form.role  !== user.role">  <span class="font-medium">Role:</span>  {{ user.role }}  → {{ form.role }}</p>
                                <p v-if="form.password">             <span class="font-medium">Password:</span> Akan diperbarui</p>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap items-center gap-3 border-t border-gray-100 pt-6 dark:border-gray-800">
                            <button type="submit" :disabled="form.processing || !hasChanges"
                                ...
                                {{ form.processing ? 'Memperbarui...' : 'Perbarui User' }}
                            </button>
