...
// ── Delete ────────────────────────────────────────────────────────
const deleteOrganisasi = (org: Organisasi) => {
    selectedOrganisasi.value = org;
...
// ── Popups (pola sama persis dengan siswa) ────────────────────────
            ...
            <div class="mx-auto max-w-7xl space-y-6 px-3 py-4 sm:px-4 sm:py-6 lg:px-8 lg:py-8">
                ...
                <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between lg:items-center">
                        ...
                        <div class="mt-3">
                            <div class="flex flex-wrap items-center gap-3 text-xs text-gray-500 dark:text-gray-400 sm:gap-6 sm:text-sm">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span>{{ organisasi.total }} Total Organisasi</span>
                                </div>
                                <div v-if="hasActiveFilters" class="flex items-center gap-2">
                                    <div class="h-2 w-2 rounded-full bg-orange-500"></div>
                                    <span>{{ organisasi.data.length }} Terfilter</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <Link :href="route('admin.organisasi.create')"
                        ...
                        <span class="sm:inline">Tambah Organisasi</span>
                    </Link>
                </div>
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-12 lg:gap-6">
                            ...
                            <div class="sm:col-span-2 lg:col-span-7">
                                ...
                                <div class="relative">
                                    ...
                                    <input v-model="searchForm.search" type="text" placeholder="Cari berdasarkan nama, jenis, deskripsi..."
                    ...
                    <div class="border-b border-gray-100 bg-gray-50/50 px-4 py-3 dark:border-gray-800 dark:bg-gray-800/50 sm:px-6 sm:py-4">
                        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-0">
                            <p class="text-sm font-medium text-gray-900 dark:text-white">
                                Menampilkan {{ organisasi.data.length }} dari {{ organisasi.total }} organisasi
                            </p>
                            <div class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                                <span>Halaman {{ organisasi.current_page }} dari {{ organisasi.last_page }}</span>
                            </div>
                        </div>
                    </div>

                    <!-- Desktop Table -->
                    <div v-if="organisasi.data.length > 0" class="hidden lg:block">
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50/80 dark:bg-gray-800/50">
                                    <tr>
                                        <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-gray-400">Organisasi</th>
                                        <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-gray-400">Jenis</th>
                                        <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-gray-400">Deskripsi</th>
                                        <th scope="col" class="relative px-6 py-4"><span class="sr-only">Aksi</span></th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-900 dark:divide-gray-700">
                                    <tr v-for="org in organisasi.data" :key="org.id"
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-4">
                                                ...
                                                <div class="min-w-0">
                                                    <div class="text-sm font-medium text-gray-900 dark:text-white truncate">{{ org.nama }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="text-sm text-gray-900 dark:text-white max-w-xs truncate" :title="org.deskripsi">
                                                {{ org.deskripsi || '-' }}
                                            </div>
                                        </td>
                        ...
                        <div v-for="org in organisasi.data" :key="org.id" class="p-4">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p v-if="org.deskripsi" class="text-xs text-gray-500 dark:text-gray-400 mt-0.5 line-clamp-2">{{ org.deskripsi }}</p>
                                </div>
                            </div>
                    ...
                    <div v-else class="p-8 text-center sm:p-16">
                        <div class="mx-auto max-w-sm">
                            ...
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                                {{ hasActiveFilters ? 'Coba sesuaikan kriteria pencarian atau bersihkan filter.' : 'Mulai dengan menambahkan organisasi pertama.' }}
                            </p>
                            <div class="mt-6 sm:mt-8">
                                <Link :href="route('admin.organisasi.create')"
                                    ...
                                    {{ hasActiveFilters ? 'Tambah Organisasi Baru' : 'Tambah Organisasi Pertama' }}
                                </Link>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Pagination -->
                <div v-if="organisasi.last_page > 1"
                    ...
                    <div class="text-sm text-gray-600 dark:text-gray-400">
                        Menampilkan
                        <span class="font-medium text-gray-900 dark:text-white">{{ ((organisasi.current_page - 1) * organisasi.per_page) + 1 }}</span>
                        sampai
                        <span class="font-medium text-gray-900 dark:text-white">{{ Math.min(organisasi.current_page * organisasi.per_page, organisasi.total) }}</span>
                        dari
                        <span class="font-medium text-gray-900 dark:text-white">{{ organisasi.total }}</span>
                        hasil
                    </div>
            ...
            <div class="relative mx-4 w-full max-w-md transform rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-black/5 transition-all dark:bg-gray-900 dark:ring-white/10">
                ...
                <div class="mt-4">
                    <p class="text-sm text-gray-700 dark:text-gray-300">
                        ...
                        <span class="font-semibold">"{{ selectedOrganisasi?.nama }}"</span>?
                    </p>
                </div>
                ...
                <div class="relative mx-4 transform transition-all duration-300 pointer-events-auto">
                    <div class="rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-black/5 dark:bg-gray-900 dark:ring-white/10 max-w-sm">
                        ...
                        <div class="mt-4 text-center">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Organisasi Berhasil Dihapus!</h3>
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Data organisasi telah dihapus secara permanen dari sistem.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-if="showSuccessCreatePopup" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto">
                ...
                <div class="relative mx-4 transform transition-all duration-300 pointer-events-auto">
                    <div class="rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-black/5 dark:bg-gray-900 dark:ring-white/10 max-w-sm">
                        ...
                        <div class="mt-4 text-center">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Organisasi Berhasil Ditambahkan!</h3>
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Data organisasi baru telah berhasil ditambahkan ke sistem.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-if="showSuccessUpdatePopup" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto">
                ...
                <div class="relative mx-4 transform transition-all duration-300 pointer-events-auto">
                    <div class="rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-black/5 dark:bg-gray-900 dark:ring-white/10 max-w-sm">
                        ...
                        <div class="mt-4 text-center">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Data Organisasi Berhasil Diperbarui!</h3>
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Perubahan data organisasi telah berhasil disimpan.</p>
                        </div>
                    </div>
                </div>
            </div>
