...
const removeFoto = () => { form.foto = null; imagePreview.value = null; if (fileInputRef.value) fileInputRef.value.value = ''; };
...
const removeImageItem = (index: number) => { imageItems.value.splice(index, 1); form.images = imageItems.value.map(i => i.file); delete localErrors.value['images']; };
...
const onSelectTanggal = (day: any) => {
    ...
    tanggalValue.value = day.date; form.tanggal_publikasi = toInputFormat(day.date); showCalendar.value = false; delete localErrors.value['tanggal_publikasi'];
};
const clearTanggal = () => { if (form.status === 'publish') return; tanggalValue.value = null; form.tanggal_publikasi = ''; };
const closeCalendar = () => { showCalendar.value = false; };

const infoStatusTanggal = computed(() => {
    if (form.status === 'publish') return { type: 'success', msg: `Akan dipublikasi langsung hari ini (${formatDisplay(today)})` };
...
const handleSubmit = () => {
    ...
    if (!valid) return;
    form.transform((data) => ({ ...data, images: imageItems.value.map(i => i.file) })).post('/admin/artikel');
};
</script>
                ...
                <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    ...
                    <form @submit.prevent="handleSubmit" class="p-6">
                        <div class="space-y-8">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="md:col-span-2 space-y-1.5">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input v-model="form.judul" @input="delete localErrors['judul']" type="text" placeholder="Masukkan judul artikel yang menarik dan informatif"
                                    ...
                                    <p v-if="localErrors.judul || form.errors.judul" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1"><span v-html="ErrIcon()"></span>{{ localErrors.judul || form.errors.judul }}</p>
                                </div>
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <div class="relative">
                                        <input v-model="form.kategori" @input="onKategoriInput" @focus="onKategoriFocus" type="text" placeholder="Ketik atau pilih kategori..." autocomplete="off"
                                        ...
                                        <div v-if="showKategoriDropdown && filteredKategori.length > 0" class="absolute left-0 top-full z-50 mt-1 w-full rounded-xl border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800 overflow-hidden">
                                            <div class="py-1 max-h-48 overflow-y-auto">
                                                <button v-for="k in filteredKategori" :key="k.value" type="button" @mousedown.prevent="selectKategori(k.value)"
                                                    ...
                                                    :class="form.kategori === k.value ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-300' : ''">{{ k.label }}</button>
                                            </div>
                                            <div v-if="form.kategori && !filteredKategori.find(k => k.value === form.kategori)" class="border-t border-gray-100 dark:border-gray-700 px-4 py-2.5">
                                        ...
                                        <div v-if="showKategoriDropdown && filteredKategori.length === 0 && form.kategori" class="absolute left-0 top-full z-50 mt-1 w-full rounded-xl border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800 px-4 py-3">
                                            <p class="text-xs text-gray-500 dark:text-gray-400">Kategori baru: <span class="font-semibold text-blue-600 dark:text-blue-400">"{{ form.kategori }}"</span></p>
                                        </div>
                                    </div>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Ketik bebas atau pilih dari daftar yang ada</p>
                                    <p v-if="localErrors.kategori || form.errors.kategori" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1"><span v-html="ErrIcon()"></span>{{ localErrors.kategori || form.errors.kategori }}</p>
                                </div>
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input v-model="form.penulis" @input="onPenulisInput" @focus="onPenulisFocus" type="text" placeholder="Ketik atau pilih nama penulis..." autocomplete="off"
                                        ...
                                        <div v-if="showPenulisDropdown && filteredPenulis.length > 0" class="absolute left-0 top-full z-50 mt-1 w-full rounded-xl border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800 overflow-hidden">
                                            <div class="py-1 max-h-48 overflow-y-auto">
                                                <button v-for="p in filteredPenulis" :key="p.value" type="button" @mousedown.prevent="selectPenulis(p.value)"
                                                    class="w-full px-4 py-2.5 text-left text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-700 dark:text-gray-300 dark:hover:bg-blue-900/20 dark:hover:text-blue-300 transition-colors"
                                                    :class="form.penulis === p.value ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-300' : ''">{{ p.label }}</button>
                                            </div>
                                            <div v-if="form.penulis && !filteredPenulis.find(p => p.value === form.penulis)" class="border-t border-gray-100 dark:border-gray-700 px-4 py-2.5">
                                        ...
                                        <div v-if="showPenulisDropdown && filteredPenulis.length === 0 && form.penulis" class="absolute left-0 top-full z-50 mt-1 w-full rounded-xl border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800 px-4 py-3">
                                            <p class="text-xs text-gray-500 dark:text-gray-400">Penulis baru: <span class="font-semibold text-blue-600 dark:text-blue-400">"{{ form.penulis }}"</span></p>
                                        </div>
                                    </div>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Ketik bebas atau pilih dari daftar yang ada</p>
                                    <p v-if="localErrors.penulis || form.errors.penulis" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1"><span v-html="ErrIcon()"></span>{{ localErrors.penulis || form.errors.penulis }}</p>
                                </div>
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <p v-if="localErrors.status || form.errors.status" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1"><span v-html="ErrIcon()"></span>{{ localErrors.status || form.errors.status }}</p>
                                </div>
                                ...
                                <div class="md:col-span-2 space-y-1.5">
                                    ...
                                    <div v-if="form.status === 'publish'" class="flex items-center gap-3 rounded-xl border-0 bg-gray-100 py-3 px-4 ring-1 ring-inset ring-gray-200 cursor-not-allowed dark:bg-gray-800 dark:ring-gray-700">
                                ...
                                <div class="md:col-span-2 space-y-1.5">
                                    ...
                                    />
                                    <p v-if="localErrors.isi || form.errors.isi" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1"><span v-html="ErrIcon()"></span>{{ localErrors.isi || form.errors.isi }}</p>
                                </div>
                            </div>
                            ...
                            <div class="space-y-1.5">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                    Foto Utama <span class="text-xs font-normal text-gray-400 ml-1">(digunakan sebagai cover/thumbnail)</span>
                                </label>
                                <div v-if="imagePreview" class="flex items-center gap-4">
                                    <img :src="imagePreview" alt="Preview foto" class="h-20 w-32 rounded-xl border border-gray-200 bg-gray-50 object-cover ring-1 ring-black/5 dark:border-gray-700 dark:bg-gray-800 dark:ring-white/10" />
                                    <div class="space-y-2">
                                ...
                                <p v-if="localErrors.foto || form.errors.foto" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1"><span v-html="ErrIcon()"></span>{{ localErrors.foto || form.errors.foto }}</p>
                            </div>
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                            Foto Tambahan <span class="text-xs font-normal text-gray-400 ml-1">(opsional, tampil sebagai galeri di detail artikel)</span>
                                        </label>
                                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ imageItems.length }}/{{ MAX_IMAGES }} foto · JPG, JPEG, PNG · Maks. 2MB per foto</p>
                                    </div>
                                    <label v-if="imageItems.length < MAX_IMAGES" for="images-input" class="inline-flex items-center gap-2 cursor-pointer rounded-xl bg-gray-100 px-4 py-2 text-xs font-semibold text-gray-700 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 transition-colors">
                                        <span v-html="PhotoIcon()"></span>Tambah Foto
                                    </label>
                                </div>
                                <div v-if="imageItems.length > 0" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                                    <div v-for="(item, index) in imageItems" :key="index" class="group relative rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 aspect-square">
                                        <img :src="item.preview" :alt="`Foto ${index + 1}`" class="w-full h-full object-cover" />
                                        <div class="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all flex items-center justify-center">
                                            <button type="button" @click="removeImageItem(index)" class="opacity-0 group-hover:opacity-100 transition-opacity bg-red-500 hover:bg-red-600 text-white rounded-full p-1.5 shadow-lg"><span v-html="XMarkIcon()"></span></button>
                                        </div>
                                    </div>
                                    <label v-if="imageItems.length < MAX_IMAGES" for="images-input" class="flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 aspect-square cursor-pointer hover:border-blue-300 hover:bg-blue-50/30 dark:hover:border-blue-700 dark:hover:bg-blue-900/10 transition-colors">
                                ...
                                <p v-if="localErrors.images || form.errors['images.0']" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1"><span v-html="ErrIcon()"></span>{{ localErrors.images || form.errors['images.0'] }}</p>
                            </div>
                        </div>
