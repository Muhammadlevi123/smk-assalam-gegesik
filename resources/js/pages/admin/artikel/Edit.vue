...
const awalTanggal = props.artikel.tanggal_publikasi
    ? props.artikel.tanggal_publikasi.split('T')[0]
    : '';

// ── Inisialisasi tanggal ──────────────────────────────────────────
const initTanggal = (): Date | null => {
    if (props.artikel.status === 'publish') {
        return parseTanggalAwal(props.artikel.tanggal_publikasi) ?? new Date(today);
    }
...
const currentFotoUrl = computed(() =>
    props.artikel.foto ? `/storage/${props.artikel.foto}` : null
);
                ...
                <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <div class="border-b border-gray-100 bg-gray-50/50 px-6 py-4 dark:border-gray-800 dark:bg-gray-800/50">
                        <div class="flex items-center gap-2">
                            ...
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Edit Informasi Artikel</h3>
                        </div>
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="rounded-xl border border-blue-200 bg-blue-50/50 p-4 dark:border-blue-800 dark:bg-blue-900/10">
                                ...
                                <div class="flex items-center gap-4">
                                    <div v-if="currentFotoUrl && !imagePreview" class="flex-shrink-0">
                                        <img :src="currentFotoUrl" :alt="artikel.judul"
                                    ...
                                    <div class="min-w-0 flex-1 text-sm text-blue-700 dark:text-blue-400 space-y-0.5">
                                        ...
                                        <p>
                                            ...
                                            <span :class="artikel.status === 'publish' ? 'text-green-700 dark:text-green-400' : 'text-orange-600 dark:text-orange-400'" class="ml-1 font-semibold">
                                                {{ artikel.status === 'publish' ? 'Publish' : 'Draft' }}
                                            </span>
                                        </p>
                                        <p v-if="artikel.tanggal_publikasi">
                                            <span class="font-medium">Tanggal Publikasi:</span>
                                            {{ formatDisplay(parseTanggalAwal(artikel.tanggal_publikasi)) }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="md:col-span-2 space-y-1.5">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input v-model="formState.judul" @input="delete localErrors['judul']"
                                            type="text" placeholder="Masukkan judul artikel"
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input v-model="formState.penulis" @input="onPenulisInput" @focus="onPenulisFocus"
                                        ...
                                        <div v-if="showPenulisDropdown && filteredPenulis.length > 0"
                                            ...
                                            <div class="py-1 max-h-48 overflow-y-auto">
                                                <button v-for="p in filteredPenulis" :key="p.value" type="button"
                                                    ...
                                                    class="w-full px-4 py-2.5 text-left text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-700 dark:text-gray-300 dark:hover:bg-blue-900/20 dark:hover:text-blue-300 transition-colors"
                                                    :class="formState.penulis === p.value ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-300' : ''">
                                                    {{ p.label }}
                                                </button>
                                            </div>
                                            <div v-if="formState.penulis && !filteredPenulis.find(p => p.value === formState.penulis)"
                                                class="border-t border-gray-100 dark:border-gray-700 px-4 py-2.5">
                                                <p class="text-xs text-gray-500 dark:text-gray-400">Gunakan penulis baru: <span class="font-semibold text-blue-600 dark:text-blue-400">"{{ formState.penulis }}"</span></p>
                                            </div>
                                        </div>
                                        <div v-if="showPenulisDropdown && filteredPenulis.length === 0 && formState.penulis"
                                            class="absolute left-0 top-full z-50 mt-1 w-full rounded-xl border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800 px-4 py-3">
                                            <p class="text-xs text-gray-500 dark:text-gray-400">Penulis baru: <span class="font-semibold text-blue-600 dark:text-blue-400">"{{ formState.penulis }}"</span></p>
                                        </div>
                                    </div>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Ketik bebas atau pilih dari daftar yang ada</p>
                                    <p v-if="localErrors.penulis || serverErrors.penulis" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1">
                                        <span v-html="ErrIcon()"></span>{{ localErrors.penulis || serverErrors.penulis }}
                                    </p>
                                </div>
                            ...
                            <div class="space-y-1.5">
                                ...
                                <div v-if="displayedFoto" class="flex items-center gap-4">
                                    <img :src="displayedFoto" alt="Foto artikel"
