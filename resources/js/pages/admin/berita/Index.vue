...
const debouncedSearch = debounce(() => {
    searchForm.get('/admin/berita', { preserveState: true, preserveScroll: true });
}, 300);

watch(
    [() => searchForm.search, () => searchForm.kategori, () => searchForm.status, () => searchForm.tahun],
    () => { debouncedSearch(); }
);
...
// ── Delete ────────────────────────────────────────────────────────
const deleteBerita = (berita: Berita) => { selectedBerita.value = berita; showDeleteModal.value = true; };

const confirmDelete = () => {
    if (!selectedBerita.value) return;
    deleteForm.delete(`/admin/berita/${selectedBerita.value.id}`, {
        preserveScroll: true,
        onSuccess: () => {
...
const hasActiveFilters = computed(() =>
    !!(props.filters?.search || props.filters?.kategori || props.filters?.status || props.filters?.tahun)
);

const countPublish = computed(() => props.berita.data.filter(b => b.status === 'publish').length);
const countDraft   = computed(() => props.berita.data.filter(b => b.status === 'draft').length);
        ...
        <div class="bg-gray-50/50 dark:bg-gray-950/50 min-h-screen">
            <div class="mx-auto max-w-7xl space-y-6 px-3 py-4 sm:px-4 sm:py-6 lg:px-8 lg:py-8">
                ...
                <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between lg:items-center">
                    <div class="space-y-2">
                        <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-3xl">Data Berita</h1>
                        <p class="text-sm text-gray-600 dark:text-gray-400 sm:text-base">Kelola berita dan konten informasi sekolah</p>
                        <div class="mt-3 flex flex-wrap items-center gap-3 text-xs text-gray-500 dark:text-gray-400 sm:gap-6 sm:text-sm">
                            <div class="flex items-center gap-2">
                                ...
                                <span>{{ berita.total }} Total Berita</span>
                            </div>
                            <div class="flex items-center gap-2">
                            ...
                            <div v-if="hasActiveFilters" class="flex items-center gap-2">
                                ...
                                <span>{{ berita.data.length }} Terfilter</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-12 xl:gap-6">
                            ...
                            <div class="sm:col-span-2 lg:col-span-3 xl:col-span-4">
                                ...
                                <div class="relative">
                                    ...
                                    <input v-model="searchForm.search" type="text" placeholder="Cari berdasarkan judul, isi, kategori..."
                ...
                <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900 sm:rounded-2xl overflow-hidden">
                    <div class="border-b border-gray-100 bg-gray-50/50 px-4 py-3 dark:border-gray-800 dark:bg-gray-800/50 sm:px-6 sm:py-4">
                        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-0">
                            <p class="text-sm font-medium text-gray-900 dark:text-white">
                                Menampilkan {{ berita.data.length }} dari {{ berita.total }} berita
                            </p>
                    ...
                    <!-- Desktop Table -->
                    <div v-if="berita.data.length > 0" class="hidden lg:block overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50/80 dark:bg-gray-800/50">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-gray-400">Berita</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-900 dark:divide-gray-700">
                                <tr v-for="item in berita.data" :key="item.id"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end gap-2">
                                            <Link :href="route('admin.berita.show', item.id)"
                                            ...
                                            <button @click="deleteBerita(item)"
                    ...
                    <!-- Mobile Card View -->
                    <div v-if="berita.data.length > 0" class="lg:hidden divide-y divide-gray-100 dark:divide-gray-800">
                        <div v-for="item in berita.data" :key="item.id" class="p-4">
                    ...
                    <div v-else class="p-8 text-center sm:p-16">
                        <div class="mx-auto max-w-sm">
                            ...
                            <h3 class="mt-4 text-lg font-semibold text-gray-900 dark:text-white sm:mt-6">Belum ada data berita</h3>
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                                {{ hasActiveFilters ? 'Coba sesuaikan kriteria pencarian atau bersihkan filter.' : 'Mulai dengan menambahkan berita pertama.' }}
                            </p>
                            <div class="mt-6 sm:mt-8">
                                <Link :href="route('admin.berita.create')"
                                    ...
                                    {{ hasActiveFilters ? 'Tambah Berita Baru' : 'Tambah Berita Pertama' }}
                                </Link>
                            </div>
                        </div>
                    </div>
                ...
                <div v-if="berita.last_page > 1"
                    ...
                    <div class="text-sm text-gray-600 dark:text-gray-400">
                        Menampilkan
                        <span class="font-medium text-gray-900 dark:text-white">{{ ((berita.current_page - 1) * berita.per_page) + 1 }}</span>
                        sampai
                        <span class="font-medium text-gray-900 dark:text-white">{{ Math.min(berita.current_page * berita.per_page, berita.total) }}</span>
                        dari
                        <span class="font-medium text-gray-900 dark:text-white">{{ berita.total }}</span>
                        hasil
                    </div>
                    <nav class="flex flex-wrap items-center justify-center gap-1 sm:justify-start" aria-label="Navigasi halaman">
                        <template v-for="link in berita.links" :key="link.label">
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto" @click.self="showDeleteModal = false">
            ...
            <div class="relative mx-4 w-full max-w-md transform rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-black/5 transition-all dark:bg-gray-900 dark:ring-white/10">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Hapus Berita</h3>
                ...
                <div class="mt-4">
                    <p class="text-sm text-gray-700 dark:text-gray-300">
                        ...
                        <span class="font-semibold">"{{ selectedBerita?.judul }}"</span>?
                    </p>
                    <div class="mt-2 rounded-lg bg-yellow-50 border border-yellow-200 p-3 dark:bg-yellow-900/20 dark:border-yellow-800">
                        <p class="text-xs text-yellow-800 dark:text-yellow-300">
                            <strong>Peringatan:</strong> Data berita dan foto akan dihapus permanen dari sistem.
                        </p>
                    </div>
                </div>
                ...
                <div class="relative mx-4 pointer-events-auto">
                    <div class="rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-black/5 dark:bg-gray-900 dark:ring-white/10 max-w-sm w-full">
                        ...
                        <div class="mt-4 text-center">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Gagal Menghapus Berita!</h3>
            ...
            <div v-if="showSuccessDeletePopup" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto">
                ...
                <div class="relative mx-4 pointer-events-auto">
                    <div class="rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-black/5 dark:bg-gray-900 dark:ring-white/10 max-w-sm w-full">
                        ...
                        <div class="mt-4 text-center">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Berita Berhasil Dihapus!</h3>
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Data berita telah dihapus secara permanen dari sistem.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-if="showSuccessCreatePopup" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto">
                ...
                <div class="relative mx-4 pointer-events-auto">
                    <div class="rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-black/5 dark:bg-gray-900 dark:ring-white/10 max-w-sm w-full">
                        ...
                        <div class="mt-4 text-center">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Berita Berhasil Ditambahkan!</h3>
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Data berita baru telah berhasil ditambahkan ke sistem.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-if="showSuccessUpdatePopup" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto">
                ...
                <div class="relative mx-4 pointer-events-auto">
                    <div class="rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-black/5 dark:bg-gray-900 dark:ring-white/10 max-w-sm w-full">
                        ...
                        <div class="mt-4 text-center">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Data Berita Berhasil Diperbarui!</h3>
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Perubahan data berita telah berhasil disimpan.</p>
                        </div>
                    </div>
                </div>
            </div>
