...
interface Guru {
    id: number;
    nama: string;
    tahun_ajaran_tersedia: number[]; // ID tahun ajaran yang guru ini berstatus Aktif
}
interface MataPelajaran {
    id: number;
    nama: string;
    pengajaran: Array<{ guru_id: string; tahun_ajaran_id: string }>;
}
interface Props { mataPelajaran: MataPelajaran; tahunAjaran: TahunAjaran[]; guru: Guru[]; }
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: `Edit ${props.mataPelajaran.nama}`, href: `/admin/mata-pelajaran/${props.mataPelajaran.id}/edit` },
];

// ── Build initial data ────────────────────────────────────────────
const buildInitialPengajaran = () => {
    const valid = props.mataPelajaran.pengajaran
        .filter(p => p.guru_id && p.tahun_ajaran_id)
        .map(p => ({ guru_id: p.guru_id.toString(), tahun_ajaran_id: p.tahun_ajaran_id.toString() }));
    return valid.length > 0 ? valid : [{ guru_id: '', tahun_ajaran_id: '' }];
};
...
// Buka semua row yang sudah ada data saat halaman dibuka
const expandedRows = ref<number[]>(formState.pengajaran.map((_, i) => i));
...
// ── Tampilkan semua guru yang Aktif di tahun ajaran tersebut ──────
// Guru boleh mengampu lebih dari satu mata pelajaran — tidak ada filter
// berdasarkan existing_pengajaran. Hanya cegah duplikat dalam form ini.
const getAvailableGuru = (taId: string, currentIndex: number) => {
    if (!taId) return [];
    const id = parseInt(taId);
    return props.guru.filter(g =>
        // Guru Aktif di tahun ajaran ini
        g.tahun_ajaran_tersedia.includes(id) &&
        // Cegah duplikat kombinasi guru+tahun dalam form yang sama
...
const hasChanges = computed(() => {
    if (formState.nama !== props.mataPelajaran.nama) return true;
    const init = buildInitialPengajaran();
    if (formState.pengajaran.length !== init.length) return true;
    for (let i = 0; i < formState.pengajaran.length; i++) {
        if (formState.pengajaran[i].guru_id         !== (init[i]?.guru_id         ?? '') ||
            formState.pengajaran[i].tahun_ajaran_id !== (init[i]?.tahun_ajaran_id ?? '')) return true;
    }
    return false;
});
...
const handleSubmit = async () => {
    ...
    formState.pengajaran.forEach((p, i) => {
        ...
        if (isEmpty) return;
        if (p.guru_id && !p.tahun_ajaran_id) {
            setError(`pengajaran_${i}_tahun`, 'Tahun ajaran wajib dipilih');
            trySet(refPengajaranRows.value[i]);
            if (!isExpanded(i)) expandedRows.value.push(i);
        }
        if (!p.guru_id && p.tahun_ajaran_id) {
            setError(`pengajaran_${i}_guru`, 'Guru wajib dipilih');
            trySet(refPengajaranRows.value[i]);
            if (!isExpanded(i)) expandedRows.value.push(i);
        }
        if (p.guru_id && p.tahun_ajaran_id && isDuplicate(p.guru_id, p.tahun_ajaran_id, i)) {
            setError(`pengajaran_${i}_dup`, 'Kombinasi guru & tahun ajaran sudah ada');
            trySet(refPengajaranRows.value[i]);
            if (!isExpanded(i)) expandedRows.value.push(i);
        }
    });
    ...
    router.post(`/admin/mata-pelajaran/${props.mataPelajaran.id}`, fd, {
        preserveScroll: true,
...
<template>
    <Head :title="`Edit Mata Pelajaran - ${mataPelajaran.nama}`" />
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="bg-gray-50/50 dark:bg-gray-950/50 min-h-screen">
            <div class="mx-auto max-w-4xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
                ...
                <div class="space-y-2">
                    <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Edit Mata Pelajaran</h1>
                    <p class="text-base text-gray-600 dark:text-gray-400">Perbarui informasi untuk mata pelajaran "{{ mataPelajaran.nama }}"</p>
                </div>
                ...
                <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <div class="border-b border-gray-100 bg-gray-50/50 px-6 py-4 dark:border-gray-800 dark:bg-gray-800/50">
                        ...
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Perbarui data mata pelajaran beserta pengajaran</p>
                    </div>
                    ...
                    <form @submit.prevent="handleSubmit" class="p-6">
                        <div class="space-y-8">
                            ...
                            <div ref="refNama" class="space-y-1.5 max-w-sm">
                                ...
                                <input v-model="formState.nama" @input="delete localErrors['nama']" type="text"
                                    ...
                                    <div v-for="(p, index) in formState.pengajaran" :key="index"
                                        ...
                                        <div v-show="isExpanded(index)" class="border-t border-green-100 bg-green-50/50 px-4 py-4 dark:border-green-900 dark:bg-green-900/5">
                                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                                ...
                                                <!-- Guru (semua yang Aktif di tahun tersebut) -->
                                                <div class="space-y-1.5">
                        ...
                        <div class="mt-8 flex flex-col gap-3 border-t border-gray-100 pt-6 dark:border-gray-800 sm:flex-row sm:items-center">
                            <button type="submit" :disabled="processing || !hasChanges"
                                ...
                                {{ processing ? 'Memperbarui...' : 'Perbarui Mata Pelajaran' }}
                            </button>
