...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit Prestasi', href: `/admin/prestasi/${props.prestasi.id}/edit` },
];

const formState = reactive({
    nama_lomba:    props.prestasi.nama_lomba,
    tingkat:       props.prestasi.tingkat,
    juara:         props.prestasi.juara,
...
const currentFotoUrl = computed(() => props.prestasi.foto ? `/storage/${props.prestasi.foto}` : null);
const displayedFoto  = computed(() => {
    if (imagePreview.value) return imagePreview.value;
    if (!fotoRemoved.value && currentFotoUrl.value) return currentFotoUrl.value;
    return null;
});
...
// new Date(string YYYY-MM-DD) → v-calendar handle dengan benar
const tanggalValue = ref<Date | null>(
    props.prestasi.tanggal ? new Date(props.prestasi.tanggal) : null
);
...
const onSelectTanggal = (day: any) => {
    ...
    formState.tanggal  = toInputFormat(day.date);
...
// Siswa
const selectedSiswaIds = ref<number[]>([...props.prestasi.siswa_prestasi]);
...
onMounted(() => {
    if (selectedSiswaIds.value.length > 0) {
        ...
        if (selected.length > 0) {
            const set = new Set(selected.map(s => s.angkatan));
            if (set.size === 1) selectedAngkatan.value = selected[0].angkatan;
        }
    }
});
...
const selectAllVisible = () => { filteredSiswa.value.forEach(s => { if (!selectedSiswaIds.value.includes(s.id)) selectedSiswaIds.value.push(s.id); }); };
const clearAllVisible  = () => { const v = filteredSiswa.value.map(s => s.id); selectedSiswaIds.value = selectedSiswaIds.value.filter(id => !v.includes(id)); };
...
const allFilteredSelected = computed(() =>
    filteredSiswa.value.length > 0 && filteredSiswa.value.every(s => selectedSiswaIds.value.includes(s.id))
);
...
const handleSubmit = () => {
    clearErrors();
    let valid = true;
    if (!formState.nama_lomba.trim()) { setError('nama_lomba', 'Nama lomba wajib diisi'); valid = false; }
    if (!formState.tingkat)           { setError('tingkat', 'Tingkat lomba wajib dipilih'); valid = false; }
    if (!formState.juara)             { setError('juara', 'Juara wajib dipilih'); valid = false; }
    if (!formState.tanggal)           { setError('tanggal', 'Tanggal lomba wajib diisi'); valid = false; }
    if (selectedSiswaIds.value.length === 0) { setError('siswa_prestasi', 'Minimal satu siswa harus dipilih'); valid = false; }
    if (!valid) return;

    const fd = new FormData();
    fd.append('_method',       'PUT');
    fd.append('nama_lomba',    formState.nama_lomba);
    fd.append('tingkat',       formState.tingkat);
    fd.append('juara',         formState.juara);
    fd.append('penyelenggara', formState.penyelenggara || '');
    fd.append('tanggal',       formState.tanggal);
    fd.append('deskripsi',     formState.deskripsi || '');
    fd.append('remove_foto',   formState.remove_foto);
    if (formState.foto instanceof File) fd.append('foto', formState.foto);
    selectedSiswaIds.value.forEach((id, i) => fd.append(`siswa_prestasi[${i}]`, id.toString()));

    processing.value = true;
    router.post(`/admin/prestasi/${props.prestasi.id}`, fd, {
        preserveScroll: true,
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="bg-gray-50/50 dark:bg-gray-950/50 min-h-screen">
            <div class="mx-auto max-w-4xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
                ...
                <div class="space-y-2">
                    <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Edit Prestasi</h1>
                    <p class="text-base text-gray-600 dark:text-gray-400">Perbarui informasi prestasi dan siswa yang meraih pencapaian ini</p>
                </div>
                ...
                <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <div class="border-b border-gray-100 bg-gray-50/50 px-6 py-4 dark:border-gray-800 dark:bg-gray-800/50">
                        <div class="flex items-center gap-2">
                            <span v-html="EditIcon()" class="text-blue-600 dark:text-blue-400"></span>
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Edit Informasi Prestasi</h3>
                        </div>
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Perbarui formulir di bawah untuk mengubah data prestasi</p>
                    </div>
                    ...
                    <form @submit.prevent="handleSubmit" class="p-6">
                        <div class="space-y-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="md:col-span-2 space-y-1.5">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input v-model="formState.nama_lomba" @input="delete localErrors['nama_lomba']"
                                            type="text" placeholder="Contoh: Olimpiade Matematika, Lomba Karya Ilmiah"
                                            :class="localErrors.nama_lomba || serverErrors.nama_lomba ? 'ring-red-400 focus:ring-red-500' : 'ring-gray-200 focus:ring-blue-600'"
                                    ...
                                    <p v-if="localErrors.nama_lomba || serverErrors.nama_lomba" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1">
                                        <span v-html="ErrIcon()"></span>{{ localErrors.nama_lomba || serverErrors.nama_lomba }}
                                    </p>
                                </div>
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <div class="relative">
                                        <select v-model="formState.tingkat" @change="delete localErrors['tingkat']"
                                            :class="localErrors.tingkat || serverErrors.tingkat ? 'ring-red-400' : 'ring-gray-200 focus:ring-blue-600'"
                                    ...
                                    <p v-if="localErrors.tingkat || serverErrors.tingkat" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1">
                                        <span v-html="ErrIcon()"></span>{{ localErrors.tingkat || serverErrors.tingkat }}
                                    </p>
                                </div>
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <div class="relative">
                                        <select v-model="formState.juara" @change="delete localErrors['juara']"
                                            :class="localErrors.juara || serverErrors.juara ? 'ring-red-400' : 'ring-gray-200 focus:ring-blue-600'"
                                            class="block w-full appearance-none rounded-xl border-0 bg-gray-50 py-3 pl-4 pr-10 text-sm text-gray-900 ring-1 ring-inset focus:bg-white focus:ring-2 dark:bg-gray-800 dark:text-white dark:ring-gray-700 dark:focus:bg-gray-700">
                                    ...
                                    <p v-if="localErrors.juara || serverErrors.juara" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1">
                                        <span v-html="ErrIcon()"></span>{{ localErrors.juara || serverErrors.juara }}
                                    </p>
                                </div>
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <input v-model="formState.penyelenggara" type="text"
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <div class="relative">
                                        <button type="button" @click="showCalendar = !showCalendar"
                                            :class="localErrors.tanggal || serverErrors.tanggal ? 'ring-red-400' : 'ring-gray-200 focus:ring-blue-600'"
                                        ...
                                        <!-- FIX: tidak ada :max-date sehingga bisa pilih masa depan -->
                                    ...
                                    <p v-if="localErrors.tanggal || serverErrors.tanggal" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1">
                                        <span v-html="ErrIcon()"></span>{{ localErrors.tanggal || serverErrors.tanggal }}
                                    </p>
                                </div>
                                ...
                                <div class="md:col-span-2 space-y-1.5">
                                    ...
                                    <textarea v-model="formState.deskripsi" rows="4"
                                    ...
                                    <p v-if="serverErrors.deskripsi" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1">
                                        <span v-html="ErrIcon()"></span>{{ serverErrors.deskripsi }}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="space-y-1.5">
                                ...
                                <div v-if="displayedFoto" class="flex items-center gap-4">
                                    <img :src="displayedFoto" alt="Foto dokumentasi"
                                ...
                                <label v-else for="foto-input"
                                    class="flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 px-6 py-8 cursor-pointer hover:border-blue-300 hover:bg-blue-50/30 transition-colors dark:border-gray-700 dark:bg-gray-800">
                            ...
                            <div class="space-y-3">
                                    ...
                                    <div class="border-b border-indigo-100 bg-indigo-50/50 px-4 py-4 dark:border-indigo-900 dark:bg-indigo-900/5 space-y-3">
                                            ...
                                            <div class="flex-1 space-y-1.5">
                                                ...
                                                <div class="relative">
                                                    ...
                                                    <input v-model="searchSiswa" type="text" placeholder="Cari nama atau NIS siswa..."
                                                        class="block w-full rounded-lg border-0 bg-white py-2.5 pl-9 pr-3 text-sm text-gray-900 ring-1 ring-inset ring-gray-200 placeholder:text-gray-400 focus:ring-2 focus:ring-indigo-600 dark:bg-gray-700 dark:text-white dark:ring-gray-600" />
                                                </div>
                                            </div>
                                        </div>
                                        <div class="flex items-center justify-between">
                                            ...
                                            <div class="flex gap-2">
                                                <button v-if="filteredSiswa.length > 0 && !allFilteredSelected" @click="selectAllVisible" type="button" class="inline-flex items-center rounded-lg bg-indigo-100 px-3 py-1.5 text-xs font-medium text-indigo-700 hover:bg-indigo-200 dark:bg-indigo-900/30 dark:text-indigo-300">Pilih Semua</button>
                                                <button v-if="filteredSiswa.length > 0 && allFilteredSelected" @click="clearAllVisible" type="button" class="inline-flex items-center rounded-lg bg-gray-100 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300">Batalkan Semua</button>
                                                <button v-if="selectedSiswaIds.length > 0" @click="clearAllSelected" type="button" class="inline-flex items-center rounded-lg bg-red-50 px-3 py-1.5 text-xs font-medium text-red-600 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-400">Hapus Pilihan</button>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <label v-for="s in filteredSiswa" :key="s.id"
                                            ...
                                            <div class="flex items-center gap-3 min-w-0 flex-1">
                                                ...
                                                <div class="min-w-0 flex-1">
                                                    ...
                                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">NIS: {{ s.nis }}<span v-if="s.existing_prestasi.length > 0" class="ml-1">· {{ s.existing_prestasi.length }} prestasi lainnya</span></p>
                                                </div>
                                            </div>
                                        </label>
                                    </div>
                                ...
                                <p v-if="localErrors.siswa_prestasi || serverErrors.siswa_prestasi" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1">
                                    <span v-html="ErrIcon()"></span>{{ localErrors.siswa_prestasi || serverErrors.siswa_prestasi }}
                                </p>
                            </div>
                        ...
                        <div class="mt-8 flex flex-col gap-3 border-t border-gray-100 pt-6 dark:border-gray-800 sm:flex-row sm:items-center">
                            <button type="submit" :disabled="processing"
                                ...
                                {{ processing ? 'Memperbarui Prestasi...' : 'Perbarui Prestasi' }}
                            </button>
