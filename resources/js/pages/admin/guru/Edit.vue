...
const buildInitialStatus = () =>
    props.guru.status_tahun_ajaran.length > 0
        ? props.guru.status_tahun_ajaran.map(s => ({ tahun_ajaran_id: s.tahun_ajaran_id.toString(), status: s.status }))
        : [{ tahun_ajaran_id: '', status: 'Aktif' }];

const buildInitialWali = () =>
    props.guru.wali_kelas.length > 0
        ? props.guru.wali_kelas.map(w => ({ kelas_id: w.kelas_id.toString(), tahun_ajaran_id: w.tahun_ajaran_id.toString() }))
        : [{ kelas_id: '', tahun_ajaran_id: '' }];

const buildInitialPengajaran = () =>
    props.guru.pengajaran.length > 0
        ? props.guru.pengajaran.map(p => ({ tahun_ajaran_id: p.tahun_ajaran_id.toString(), nama_mata_pelajaran: p.nama_mata_pelajaran }))
        : [{ tahun_ajaran_id: '', nama_mata_pelajaran: '' }];
...
const refNip        = ref<HTMLElement | null>(null);
...
// ── Computed: tahun ajaran aktif dari pilihan status ──────────────
const tahunAjaranAktif = computed(() => {
    const aktifIds = formState.status_tahun_ajaran
...
const removeStatus = (i: number) => {
    if (formState.status_tahun_ajaran.length > 1) {
        ...
        if (removedStatus === 'Aktif' && removedId) {
            ...
            if (!masihAktif) {
                formState.wali_kelas.forEach(w => { if (w.tahun_ajaran_id === removedId) { w.tahun_ajaran_id = ''; w.kelas_id = ''; } });
                formState.pengajaran.forEach(p => { if (p.tahun_ajaran_id === removedId) { p.tahun_ajaran_id = ''; p.nama_mata_pelajaran = ''; } });
            }
        }
    }
};
const onStatusTahunChange = (index: number, oldId: string) => {
    ...
    if (!masihAktif) {
        formState.wali_kelas.forEach(w => { if (w.tahun_ajaran_id === oldId) { w.tahun_ajaran_id = ''; w.kelas_id = ''; } });
        formState.pengajaran.forEach(p => { if (p.tahun_ajaran_id === oldId) { p.tahun_ajaran_id = ''; p.nama_mata_pelajaran = ''; } });
    }
};
const onStatusValueChange = (index: number) => {
    ...
    if (item.status !== 'Aktif' && item.tahun_ajaran_id) {
        ...
        if (!masihAktif) {
            formState.wali_kelas.forEach(w => { if (w.tahun_ajaran_id === id) { w.tahun_ajaran_id = ''; w.kelas_id = ''; } });
            formState.pengajaran.forEach(p => { if (p.tahun_ajaran_id === id) { p.tahun_ajaran_id = ''; p.nama_mata_pelajaran = ''; } });
        }
    }
};
const getAvailableTahunForStatus = (currentIndex: number) => {
    const used = formState.status_tahun_ajaran.filter((_, i) => i !== currentIndex).map(s => s.tahun_ajaran_id).filter(Boolean);
    return props.tahunAjaran.filter(t => !used.includes(t.id.toString()));
};
const isDupStatus = (taId: string, idx: number) =>
    !!(taId && formState.status_tahun_ajaran.some((s, i) => i !== idx && s.tahun_ajaran_id === taId));

// ── Wali kelas helpers ────────────────────────────────────────────
const addWali    = () => formState.wali_kelas.push({ kelas_id: '', tahun_ajaran_id: '' });
const removeWali = (i: number) => {
    if (formState.wali_kelas.length > 1) formState.wali_kelas.splice(i, 1);
    else formState.wali_kelas[0] = { kelas_id: '', tahun_ajaran_id: '' };
};
const onWaliTahunChange = (i: number) => {
    formState.wali_kelas[i].kelas_id = '';
    delete localErrors.value[`wali_${i}_kelas`];
    delete localErrors.value[`wali_${i}_tahun`];
};
const getAvailableKelas = (taId: string, currentIndex: number) => {
    if (!taId) return [];
    const id = parseInt(taId);
    return props.kelas.filter(k =>
        !k.existing_wali_tahun_ajaran.includes(id) &&
        !formState.wali_kelas.some((w, i) => i !== currentIndex && w.kelas_id === k.id.toString() && w.tahun_ajaran_id === taId)
    );
};
const getAvailableTahunForWali = (currentIndex: number) => {
    const used = formState.wali_kelas.filter((_, i) => i !== currentIndex).map(w => w.tahun_ajaran_id).filter(Boolean);
    return tahunAjaranAktif.value.filter(t => !used.includes(t.id.toString()));
};
const isDupWali = (kelasId: string, taId: string, idx: number) =>
    !!(kelasId && taId && formState.wali_kelas.some((w, i) => i !== idx && w.kelas_id === kelasId && w.tahun_ajaran_id === taId));

// ── Pengajaran helpers ────────────────────────────────────────────
const addPengaj    = () => formState.pengajaran.push({ tahun_ajaran_id: '', nama_mata_pelajaran: '' });
const removePengaj = (i: number) => {
    if (formState.pengajaran.length > 1) formState.pengajaran.splice(i, 1);
    else formState.pengajaran[0] = { tahun_ajaran_id: '', nama_mata_pelajaran: '' };
};
const onPengajTahunChange = (i: number) => {
    formState.pengajaran[i].nama_mata_pelajaran = '';
    delete localErrors.value[`pengaj_${i}_tahun`];
};
const getAvailableTahunForPengaj = () => tahunAjaranAktif.value;
const isPengajPartial = (p: { tahun_ajaran_id: string; nama_mata_pelajaran: string }) =>
    (p.tahun_ajaran_id !== '' && !p.nama_mata_pelajaran) ||
    (!p.tahun_ajaran_id && p.nama_mata_pelajaran !== '');

// ── hasChanges ────────────────────────────────────────────────────
const hasChanges = computed(() => {
    if (formState.nama          !== props.guru.nama)          return true;
    if (formState.nip           !== props.guru.nip)           return true;
    ...
    const currentWali = formState.wali_kelas
        .filter(w => w.kelas_id && w.tahun_ajaran_id)
        .map(w => `${w.kelas_id}:${w.tahun_ajaran_id}`).sort().join('|');
    const originalWali = (props.guru.wali_kelas || [])
        .map(w => `${w.kelas_id}:${w.tahun_ajaran_id}`).sort().join('|');
    if (currentWali !== originalWali) return true;

    const currentPengaj = formState.pengajaran
        .filter(p => p.tahun_ajaran_id && p.nama_mata_pelajaran)
        .map(p => `${p.tahun_ajaran_id}:${p.nama_mata_pelajaran}`).sort().join('|');
    const originalPengaj = (props.guru.pengajaran || [])
        .map(p => `${p.tahun_ajaran_id}:${p.nama_mata_pelajaran}`).sort().join('|');
...
const handleSubmit = async () => {
    ...
    if (!formState.nip.trim())    { setError('nip', 'NIP wajib diisi');               trySet(refNip.value); }
    ...
    formState.wali_kelas.forEach((w, i) => {
        ...
        if (partial) {
            if (w.kelas_id && !w.tahun_ajaran_id) setError(`wali_${i}_tahun`, 'Tahun ajaran wajib dipilih');
            else setError(`wali_${i}_kelas`, 'Kelas wajib dipilih');
            trySet(refWaliRows.value[i]);
        }
    ...
    formState.pengajaran.forEach((p, i) => {
        if (isPengajPartial(p)) {
            if (p.tahun_ajaran_id && !p.nama_mata_pelajaran) setError(`pengaj_${i}_mapel`, 'Mata pelajaran wajib dipilih');
            else setError(`pengaj_${i}_tahun`, 'Tahun ajaran wajib dipilih');
            trySet(refPengajRows.value[i]);
        }
    });
    ...
    let wi = 0;
    formState.wali_kelas.forEach(w => {
        if (w.kelas_id && w.tahun_ajaran_id) {
            fd.append(`wali_kelas[${wi}][kelas_id]`,        w.kelas_id);
            fd.append(`wali_kelas[${wi}][tahun_ajaran_id]`, w.tahun_ajaran_id);
            wi++;
        }
    });

    let pi = 0;
    formState.pengajaran.forEach(p => {
        if (p.tahun_ajaran_id && p.nama_mata_pelajaran) {
            fd.append(`pengajaran[${pi}][nama_mata_pelajaran]`, p.nama_mata_pelajaran);
            fd.append(`pengajaran[${pi}][tahun_ajaran_id]`,     p.tahun_ajaran_id);
            pi++;
        }
    });
                            ...
                            <div class="p-6 space-y-4">
                                <div class="flex justify-center">
                                    <div>
                                        ...
                                        <div v-else>
                                            <img :src="imagePreview || getCurrentImageUrl()" :alt="guru.nama"
                                ...
                                <button v-if="(guru.foto && !hapusFoto) || imagePreview" @click="imagePreview ? removeImage() : hapusFotoAction()" type="button"
                                ...
                                <button v-if="hapusFoto && !imagePreview" @click="hapusFoto = false; existingFoto = guru.foto ? `/storage/${guru.foto}` : null" type="button"
                            ...
                            <form @submit.prevent="handleSubmit" class="p-6">
                                <div class="space-y-8">
                                    ...
                                    <div class="rounded-xl border border-gray-200 bg-gray-50/50 p-5 dark:border-gray-700 dark:bg-gray-800/50">
                                        ...
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div class="min-w-0 flex-1">
                                                ...
                                                <div class="text-sm text-gray-600 dark:text-gray-400">NIP: {{ guru.nip }} · {{ guru.jenis_kelamin }}</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div ref="refNip" class="space-y-1.5">
                                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">NIP <span class="text-red-500">*</span></label>
                                            <input v-model="formState.nip" @input="delete localErrors['nip']" type="text" placeholder="Nomor Induk Pegawai"
                                                :class="localErrors.nip || serverErrors.nip ? 'ring-red-400 focus:ring-red-500' : 'ring-gray-200 focus:ring-blue-600'"
                                                class="block w-full rounded-xl border-0 bg-gray-50 py-3 px-4 text-gray-900 ring-1 ring-inset placeholder:text-gray-400 focus:bg-white focus:ring-2 dark:bg-gray-800 dark:text-white dark:ring-gray-700 dark:placeholder:text-gray-500 dark:focus:bg-gray-700" />
                                            <p v-if="localErrors.nip || serverErrors.nip" class="text-xs text-red-500 dark:text-red-400 flex items-center gap-1"><span v-html="ErrIcon()"></span>{{ localErrors.nip || serverErrors.nip }}</p>
                                        </div>
                                    ...
                                    <div class="space-y-1.5">
                                        ...
                                        <textarea v-model="formState.alamat" rows="3" placeholder="Masukkan alamat lengkap guru"
                                        ...
                                        <div class="flex items-center justify-between flex-wrap gap-3">
                                            <div>
                                                ...
                                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                                    Status <span class="font-semibold text-green-700 dark:text-green-400">Aktif</span> membuka bagian Wali Kelas dan Mata Pelajaran.
                                                </p>
                                            </div>
                                            <button @click="addStatus" type="button"
                                        ...
                                        <div class="space-y-3">
                                            <div v-for="(s, index) in formState.status_tahun_ajaran" :key="index"
                                                ...
                                                <div class="md:col-span-2 space-y-1.5">
                                                    ...
                                                    <div class="relative">
                                                        <select :value="s.tahun_ajaran_id"
                                                            @change="(e) => {
                                                                const oldId = s.tahun_ajaran_id;
                                                                s.tahun_ajaran_id = (e.target as HTMLSelectElement).value;
                                                ...
                                                <div class="md:col-span-1 space-y-1.5">
                                                    ...
                                                    <div class="relative">
                                                        <select v-model="s.status" @change="onStatusValueChange(index)"
                                                ...
                                                <div class="flex items-start pt-5">
                                                    <button v-if="formState.status_tahun_ajaran.length > 1" @click="removeStatus(index)" type="button"
                                    ...
                                    <div class="space-y-4">
                                        <div class="flex items-center justify-between flex-wrap gap-3">
                                            <div>
                                                <h4 class="text-sm font-semibold" :class="hasStatusAktifDipilih ? 'text-gray-700 dark:text-white' : 'text-gray-400 dark:text-gray-500'">
                                                    Wali Kelas <span class="text-xs font-normal text-gray-400 ml-1">(opsional)</span>
                                                </h4>
                                            ...
                                            <button v-if="hasStatusAktifDipilih" @click="addWali" type="button"
                                                class="inline-flex items-center gap-2 rounded-lg bg-purple-50 px-3 py-2 text-sm font-medium text-purple-700 hover:bg-purple-100 dark:bg-purple-900/20 dark:text-purple-400 dark:hover:bg-purple-900/40">
                                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
                                                Tambah
                                            </button>
                                        </div>
                                        ...
                                        <div v-else class="space-y-3">
                                            <div v-for="(w, index) in formState.wali_kelas" :key="index"
                                                ...
                                                <!-- Tahun Ajaran (hanya dari status Aktif) -->
                                                <div class="md:col-span-2 space-y-1.5">
                                                    <label class="block text-xs font-medium text-gray-700 dark:text-gray-300">Tahun Ajaran</label>
                                                    <div class="relative">
                                                        <select v-model="w.tahun_ajaran_id" @change="onWaliTahunChange(index)"
                                                            :class="localErrors[`wali_${index}_tahun`] ? 'ring-red-400' : 'ring-gray-200'"
                                                            class="block w-full appearance-none rounded-lg border-0 bg-white py-2 pl-3 pr-8 text-sm text-gray-900 ring-1 ring-inset focus:ring-2 focus:ring-purple-600 dark:bg-gray-700 dark:text-white dark:ring-gray-600">
                                                            <option value="">Pilih tahun ajaran</option>
                                                ...
                                                <!-- Kelas (disabled sampai tahun dipilih) -->
                                                <div class="md:col-span-2 space-y-1.5">
                                                    <label class="block text-xs font-medium text-gray-700 dark:text-gray-300">Kelas</label>
                                                    <div class="relative">
                                                        <select v-model="w.kelas_id" :disabled="!w.tahun_ajaran_id"
                                                            @change="delete localErrors[`wali_${index}_kelas`]"
                                                            :class="localErrors[`wali_${index}_kelas`] || localErrors[`wali_${index}_dup`] ? 'ring-red-400' : 'ring-gray-200'"
                                                            class="block w-full appearance-none rounded-lg border-0 bg-white py-2 pl-3 pr-8 text-sm text-gray-900 ring-1 ring-inset focus:ring-2 focus:ring-purple-600 disabled:bg-gray-100 disabled:cursor-not-allowed dark:bg-gray-700 dark:text-white dark:ring-gray-600 dark:disabled:bg-gray-800">
                                                ...
                                                <div class="flex items-start pt-5">
                                                    <button @click="removeWali(index)" type="button"
                                                        class="ml-auto inline-flex items-center justify-center rounded-lg bg-red-50 p-2 text-red-700 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-400">
                                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                                    </button>
                                                </div>
                                    ...
                                    <div class="space-y-4">
                                        <div class="flex items-center justify-between flex-wrap gap-3">
                                            <div>
                                                <h4 class="text-sm font-semibold" :class="hasStatusAktifDipilih ? 'text-gray-700 dark:text-white' : 'text-gray-400 dark:text-gray-500'">
                                                    Mata Pelajaran yang Diampu <span class="text-xs font-normal text-gray-400 ml-1">(opsional)</span>
                                                </h4>
                                                <p class="text-sm mt-1" :class="hasStatusAktifDipilih ? 'text-gray-500 dark:text-gray-400' : 'text-gray-400 dark:text-gray-500'">
                                            ...
                                            <button v-if="hasStatusAktifDipilih" @click="addPengaj" type="button"
                                                class="inline-flex items-center gap-2 rounded-lg bg-indigo-50 px-3 py-2 text-sm font-medium text-indigo-700 hover:bg-indigo-100 dark:bg-indigo-900/20 dark:text-indigo-400 dark:hover:bg-indigo-900/40">
                                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
                                                Tambah
                                            </button>
                                        </div>
                                        ...
                                        <div v-else class="space-y-3">
                                            <div v-for="(p, index) in formState.pengajaran" :key="index"
                                                ...
                                                <div class="md:col-span-2 space-y-1.5">
                                                    <label class="block text-xs font-medium text-gray-700 dark:text-gray-300">Tahun Ajaran</label>
                                                    <div class="relative">
                                                        <select v-model="p.tahun_ajaran_id" @change="onPengajTahunChange(index)"
                                                            :class="localErrors[`pengaj_${index}_tahun`] ? 'ring-red-400' : 'ring-gray-200'"
                                                            class="block w-full appearance-none rounded-lg border-0 bg-white py-2 pl-3 pr-8 text-sm text-gray-900 ring-1 ring-inset focus:ring-2 focus:ring-indigo-600 dark:bg-gray-700 dark:text-white dark:ring-gray-600">
                                                            <option value="">Pilih tahun ajaran</option>
                                                ...
                                                <!-- Mata Pelajaran (disabled sampai tahun dipilih) -->
                                                <div class="md:col-span-2 space-y-1.5">
                                                    <label class="block text-xs font-medium text-gray-700 dark:text-gray-300">Mata Pelajaran</label>
                                                    <div class="relative">
                                                        <select v-model="p.nama_mata_pelajaran"
                                                            :disabled="!p.tahun_ajaran_id"
                                                            @change="delete localErrors[`pengaj_${index}_mapel`]"
                                                            :class="localErrors[`pengaj_${index}_mapel`] ? 'ring-red-400' : 'ring-gray-200'"
                                                            class="block w-full appearance-none rounded-lg border-0 bg-white py-2 pl-3 pr-8 text-sm text-gray-900 ring-1 ring-inset focus:ring-2 focus:ring-indigo-600 disabled:bg-gray-100 disabled:cursor-not-allowed dark:bg-gray-700 dark:text-white dark:ring-gray-600 dark:disabled:bg-gray-800">
                                                ...
                                                <div class="flex items-start pt-5">
                                                    <button @click="removePengaj(index)" type="button"
                                                        class="ml-auto inline-flex items-center justify-center rounded-lg bg-red-50 p-2 text-red-700 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-400">
                                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                                    </button>
                                                </div>
                                ...
                                <div class="mt-8 flex items-center gap-4 border-t border-gray-100 pt-6 dark:border-gray-800">
                                    ...
                                    <Link :href="previous_url || '/admin/guru'"
