...
const formState = reactive({
    ...
    wali_kelas:  [{ kelas_id: '', tahun_ajaran_id: '' }] as Array<{ kelas_id: string; tahun_ajaran_id: string }>,
    pengajaran:  [{ tahun_ajaran_id: '', nama_mata_pelajaran: '' }] as Array<{ tahun_ajaran_id: string; nama_mata_pelajaran: string }>,
...
const getAvailableKelasForWali = (taId: string, currentIndex: number) => {
    if (!taId) return [];
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="bg-gray-50/50 dark:bg-gray-950/50 min-h-screen">
            <div class="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
                ...
                <div class="space-y-2">
                    <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Tambah Guru Baru</h1>
                    <p class="text-base text-gray-600 dark:text-gray-400">Tambahkan data guru atau tenaga pengajar baru ke dalam sistem sekolah</p>
                </div>
                    ...
                    <div class="xl:col-span-2">
                        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                            <div class="border-b border-gray-100 bg-gray-50/50 px-6 py-4 dark:border-gray-800 dark:bg-gray-800/50">
                                ...
                                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Lengkapi formulir di bawah untuk menambahkan guru baru.</p>
                            </div>
                            ...
                            <form @submit.prevent="handleSubmit" class="p-6">
                                                ...
                                                <div class="md:col-span-2 space-y-1.5">
                                                    ...
                                                    <div class="relative">
                                                        <select v-model="w.kelas_id" :disabled="!w.tahun_ajaran_id"
                                                            ...
                                                            <option v-for="k in getAvailableKelasForWali(w.tahun_ajaran_id, index)" :key="k.id" :value="k.id.toString()">{{ k.nama_kelas }}</option>
                                                        </select>
                                                        <div class="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none"><span v-html="ChevronDownIcon()" class="text-gray-400"></span></div>
                                                    </div>
                                                    <p v-if="w.tahun_ajaran_id" class="text-xs text-blue-600 dark:text-blue-400">{{ getAvailableKelasForWali(w.tahun_ajaran_id, index).length }} kelas tersedia</p>
                                ...
                                <div class="mt-8 flex items-center gap-4 border-t border-gray-100 pt-6 dark:border-gray-800">
                                    <button type="submit" :disabled="processing"
                                        ...
                                        {{ processing ? 'Menyimpan Data...' : 'Simpan Guru' }}
                                    </button>
