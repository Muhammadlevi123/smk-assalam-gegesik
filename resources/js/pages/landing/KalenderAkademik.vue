...
// ─── Map tanggal → event (dengan filter include_weekend) ──────────
const eventPadaTanggal = computed(() => {
    ...
    props.kalender?.forEach(item => {
        ...
        while (cur <= selesai) {
            ...
            // ✅ Filter: kalau include_weekend = false, skip Sabtu (6) dan Minggu (0)
...
const tutupPopover = () => { showPopover.value = false; selectedDate.value = null; anchorEl.value = null; };
...
<template>
    ...
    <div class="page-root">
                    ...
                    <template v-else>
                        ...
                        <div class="cal-wrapper fade-in">
                            ...
                            <div class="cal-grid">
                                ...
                                <div v-for="(sel, idx) in selKalender" :key="idx"
                                    class="cal-cell"
                                    :class="{
                                        'cal-empty':    !sel.tgl,
        ...
        <Transition name="pop">
            <div v-if="showPopover && selectedEvents.length > 0"
                ref="popoverEl"
                class="popover-box"
...
<style>
.popover-box{position:fixed;z-index:999;background:white;border-radius:10px;box-shadow:0 4px 24px rgba(0,0,0,0.15),0 1px 4px rgba(0,0,0,0.08);border:1px solid #e5e7eb;overflow:visible}
.popover-box.arr-down::after{content:'';position:absolute;bottom:-12px;left:50%;transform:translateX(-50%);width:0;height:0;border-left:12px solid transparent;border-right:12px solid transparent;border-top:12px solid white;filter:drop-shadow(0 2px 2px rgba(0,0,0,0.08))}
.popover-box.arr-down::before{content:'';position:absolute;bottom:-14px;left:50%;transform:translateX(-50%);width:0;height:0;border-left:13px solid transparent;border-right:13px solid transparent;border-top:13px solid #e5e7eb}
.popover-box.arr-up::after{content:'';position:absolute;top:-12px;left:50%;transform:translateX(-50%);width:0;height:0;border-left:12px solid transparent;border-right:12px solid transparent;border-bottom:12px solid white;filter:drop-shadow(0 -2px 2px rgba(0,0,0,0.06))}
.popover-box.arr-up::before{content:'';position:absolute;top:-14px;left:50%;transform:translateX(-50%);width:0;height:0;border-left:13px solid transparent;border-right:13px solid transparent;border-bottom:13px solid #e5e7eb}
.pop-header{display:flex;align-items:center;justify-content:space-between;padding:11px 14px 10px;border-bottom:1px solid #f3f4f6;border-radius:10px 10px 0 0;background:white}
.pop-date{font-size:13px;font-weight:700;color:#15803d;font-family:'Plus Jakarta Sans',sans-serif}
.pop-close{width:22px;height:22px;border-radius:6px;border:none;background:#f3f4f6;color:#6b7280;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .2s;flex-shrink:0}
