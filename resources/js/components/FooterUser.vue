<script setup lang="ts">
import { Link } from '@inertiajs/vue3';
</script>

<template>
    <!-- Footer -->
    <footer class="bg-gray-800 text-white py-12" id="contact-section">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <!-- Main Footer Content -->
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">

                <!-- Left Side - Logo and School Info -->
<div class="space-y-4">
    <!-- Logo -->
    <div class="flex items-center gap-3">
        <Link href="/" class="flex items-center">
            <div class="flex items-center justify-center overflow-hidden
                         lg:h-16 lg:w-90 h-20 w-64 flex-shrink-0">
                <img
                    src="/storage/img/logo/logo-white.png"
                    alt="Logo Sekolah"
                    class="object-contain h-full w-full"
                />
            </div>
        </Link>
    </div>

    <!-- School Address -->
    <div class="text-gray-300 space-y-2 text-sm">
        <p>Gegesik Lor, Kec. Gegesik</p>
        <p>Kabupaten Cirebon, Jawa Barat</p>
    </div>
</div>
                <!-- Center - Quick Links/Features -->
                <div>
                    <h4 class="text-lg font-semibold mb-4 text-white">Tautan</h4>
                    <div class="space-y-3 text-gray-300">
                        <Link href="/profil/sejarah" class="block hover:text-white transition-colors">Tentang Kami</Link>
                        <Link href="/profil/prestasi" class="block hover:text-white transition-colors">Prestasi</Link>
                        <Link href="/informasi/artikel" class="block hover:text-white transition-colors">Artikel</Link>
                        <Link href="/informasi/berita" class="block hover:text-white transition-colors">Berita</Link>
                        <Link href="/informasi/kalender-akademik" class="block hover:text-white transition-colors">Kalender Akademik
                        </Link>
                    </div>
                </div>

                <!-- Right Side - Social Media -->
                <div>
                    <h4 class="text-lg font-semibold mb-4 text-white">Sosial Media</h4>
                    <div class="space-y-3">
                        <!-- Facebook -->
                        <a href="https://www.facebook.com/smkassalamgegesik2003"
                            class="flex items-center space-x-3 text-gray-300 hover:text-white transition-colors group">
                            <div
                                class="w-8 h-8 bg-gray-700 rounded-full flex items-center justify-center group-hover:bg-gray-600 transition-colors">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                                        d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                                </svg>
                            </div>
                            <span>Facebook</span>
                        </a>

                        <!-- Instagram -->
                        <a href="https://www.instagram.com/smkassalamgegesikcirebon?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw=="
                            class="flex items-center space-x-3 text-gray-300 hover:text-white transition-colors group">
                            <div
                                class="w-8 h-8 bg-gray-700 rounded-full flex items-center justify-center group-hover:bg-gray-600 transition-colors">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                                        d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                                </svg>
                            </div>
                            <span>Instagram</span>
                        </a>

                        <!-- YouTube -->
                        <a href="https://www.youtube.com/@smkassalamgegesik"
                            class="flex items-center space-x-3 text-gray-300 hover:text-white transition-colors group">
                            <div
                                class="w-8 h-8 bg-gray-700 rounded-full flex items-center justify-center group-hover:bg-gray-600 transition-colors">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                                        d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                                </svg>
                            </div>
                            <span>YouTube</span>
                        </a>
                    </div>
                </div>
            </div>

            <!-- Copyright Section -->
            <div class="border-t border-gray-700 pt-8">
                <div class="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
                    <div class="text-gray-300 text-sm">
                        <p>&copy; 2025 SMK Assalam Gegesik. All rights reserved. <span
                                class="text-gray-800 break-all">by Levi</span></p>
                    </div>
                    <!-- <div class="flex space-x-4 text-gray-300 text-sm">
                    <Link href="/privacy" class="hover:text-white transition-colors">Privacy Policy</Link>
                    <span>|</span>
                    <Link href="/terms" class="hover:text-white transition-colors">Terms of Service</Link>
                    <span>|</span>
                    <Link href="/sitemap" class="hover:text-white transition-colors">Site Map</Link>
                </div> -->
                </div>
            </div>

            <!-- Mobile Contact Info (Show only on mobile) -->
            <div class="bg-gray-700 rounded-lg p-4 mt-8 md:hidden">
                <div class="space-y-3 text-sm">
                    <div class="flex items-center space-x-3">
                        <svg class="w-4 h-4 text-gray-300" fill="currentColor" viewBox="0 0 24 24">
                            <path
                                d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z" />
                        </svg>
                        <span class="text-gray-300">+62 21 8459.7175</span>
                    </div>
                    <div class="flex items-center space-x-3">
                        <svg class="w-4 h-4 text-gray-300" fill="currentColor" viewBox="0 0 24 24">
                            <path
                                d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z" />
                        </svg>
                        <span class="text-gray-300 break-all">info@smkassalamgegesik.sch.id</span>
                    </div>
                </div>
            </div>
        </div>
    </footer>
</template>
